...
/**
 * RefRegion is a servlet controller for ref_region Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class RefRegionController implements Controller
...

{
	...
	public ModelAndView deletePerformed (HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
						java.io.Serializable pkey = refRegionId;
						
			ActionUser user = securityService.getActionUser(request);
		
			RefRegion res = refRegionService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.refregion",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.refregion",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchRefRegion");
		}
		catch (Exception e){
	    ...
	    try {
			...
			*/
			...
			result = new ModelAndView (location);
						java.io.Serializable pkey = refRegionId;
				        RefRegion object = refRegionService.get (pkey);

			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.refregion",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("refRegion", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("regionName")){
					vLikeP.add("regionName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("regionCode")){
					vLikeP.add("regionCode");
					vLikeQ.add(searchtext);
				}
			...
			count = refRegionService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = refRegionService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("RefRegions",collection);
	...
	public ModelAndView lookupJsonPerformed(HttpServletRequest request,
		...
		try {
			...
			Collection<RefRegion> cities = null; 
				
			if (countryId == null && countryName.equalsIgnoreCase("")){
				cities = refRegionService.searchRegion(q.toLowerCase());
			}
			else if (!countryName.equalsIgnoreCase("")){
				cities = refRegionService.searchRegion(q.toLowerCase(), countryName) ;
			}
			else if (countryId != null) {
				cities = refRegionService.searchRegion(q.toLowerCase(),countryId);
			}
			
			
			result.addObject("Regions", cities);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailRefRegion" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupRefRegion" );
	    }
	    else if (navigation.equalsIgnoreCase("lookupjson")){
	    	result = lookupJsonPerformed(request, response, "lookupJsonRefRegion");
	    }
	    else {
	    	result = searchPerformed (request, response, "searchRefRegion" );
	    }
