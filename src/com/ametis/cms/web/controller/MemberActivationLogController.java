...
*/
public class MemberActivationLogController implements Controller
...

{
	...
	private MemberActivationLogService  memberActivationLogService;
	...
	public void setMemberActivationLogService	(MemberActivationLogService memberActivationLogService){
	this.memberActivationLogService = memberActivationLogService;
	}
	public MemberActivationLogService getMemberActivationLogService (){
	return this.memberActivationLogService;
	}
	public ModelAndView deletePerformed (HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ModelAndView result = null;

 	    try {
									Long memberActivationLogId = WebUtil.getParameterLong (request, "memberActivationLogId");
			...
			*/
						java.io.Serializable pkey = memberActivationLogId;
						
			ActionUser user = securityService.getActionUser(request);
		
			MemberActivationLog res = memberActivationLogService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.memberactivationlog",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.memberactivationlog",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchMemberActivationLog");
		}
		catch (Exception e){
	    ...
	    try {
			...
			*/
			...
			result = new ModelAndView (location);
						java.io.Serializable pkey = memberActivationLogId;
				        MemberActivationLog object = memberActivationLogService.get (pkey);

			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.memberactivationlog",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("memberActivationLog", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = memberActivationLogService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = memberActivationLogService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("MemberActivationLogs",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailMemberActivationLog" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupMemberActivationLog" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchMemberActivationLog" );
	    }
