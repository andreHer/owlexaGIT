...
*/
public class ProductTypePoliklinikController implements Controller
...

{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProductTypePoliklinik res = productTypePoliklinikService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.producttypepoliklinik",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.producttypepoliklinik",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProductTypePoliklinik");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.producttypepoliklinik",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("productTypePoliklinik", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = productTypePoliklinikService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"ProductTypePoliklinik.ProductTypeId",
				"ProductTypePoliklinik.PoliklinikId",
		// foreign affairs end
	};
		    collection = productTypePoliklinikService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProductTypePolikliniks",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProductTypePoliklinik" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProductTypePoliklinik" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProductTypePoliklinik" );
	    }
