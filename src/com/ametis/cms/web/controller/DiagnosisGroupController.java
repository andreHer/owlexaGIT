...
*/
public class DiagnosisGroupController implements Controller
...
// extends- 
{
	...
	public void setDiagnosisGroupService	(DiagnosisGroupService diagnosisGroupService){
	this.diagnosisGroupService = diagnosisGroupService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			DiagnosisGroup res = diagnosisGroupService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.diagnosisgroup",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.diagnosisgroup",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchDiagnosisGroup");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.diagnosisgroup",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("diagnosisGroup", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("diagnosisGroupCode")){
					vLikeP.add("diagnosisGroupCode");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("diagnosisGroupCategory")){
					vLikeP.add("diagnosisGroupCategory");
					vLikeQ.add(searchtext);
				}
							    			}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = diagnosisGroupService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = diagnosisGroupService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("DiagnosisGroups",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailDiagnosisGroup" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupDiagnosisGroup" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchDiagnosisGroup" );
	    }
