...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			Notification res = notificationService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.notification",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.notification",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchNotification");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.notification",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("notification", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("destination")){
					vLikeP.add("destination");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("sender")){
					vLikeP.add("sender");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = notificationService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = notificationService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Notifications",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailNotification" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupNotification" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchNotification" );
	    }
