...
/**
 * Broker is a servlet controller for broker Table.
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			Broker res = brokerService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.broker",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.broker",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchBroker");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.broker",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("broker", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("brokerName")){
					vLikeP.add("brokerName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("brokerCode")){
					vLikeP.add("brokerCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = brokerService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = brokerService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Brokers",collection);
	...
	public ModelAndView lookupJsonPerformed (HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ModelAndView result = null;
		try {
		    result = new ModelAndView ("lookupBrokerJson");
			...
			String[] likeParam = {"brokerName"};
			Object[] likeValue = {q};
			
			Collection<Broker> brokerList = brokerService.search(likeParam,likeValue,eqParam,eqValue,0,15);

		    result.addObject("Brokers",brokerList);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailBroker" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupBroker" );
	    }
