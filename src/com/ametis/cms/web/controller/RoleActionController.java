...
/**
 * RoleAction is a servlet controller for role_action Table. All you have to do
 * is to convert necessary data field to the named parameter
 */
public class RoleActionController implements Controller
...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer roleActionId = WebUtil.getParameterInteger(request,
					"roleActionId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEROLEACTION");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEROLEACTION access");
				return errorResult;
				
			}
			RoleAction res = roleActionService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.roleaction", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.roleaction", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchRoleAction");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.roleaction", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("roleAction", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			count = roleActionService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			// foreign affairs
					"RoleAction.ActionId", "RoleAction.RoleId",
			// foreign affairs end
			};
			collection = roleActionService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = roleActionService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("RoleActions", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailRoleAction");
				...
				result = searchPerformed(request, response, "lookupRoleAction");
			} else {
				result = searchPerformed(request, response, "searchRoleAction");
			}
		} catch (Exception e) {
