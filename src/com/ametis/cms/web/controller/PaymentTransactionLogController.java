...
/**
 * PaymentTransactionLog is a servlet controller for payment_transaction_log Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class PaymentTransactionLogController implements Controller
...
// extends- 
{
	...
	public void setPaymentTransactionLogService	(PaymentTransactionLogService paymentTransactionLogService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			PaymentTransactionLog res = paymentTransactionLogService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.paymenttransactionlog",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.paymenttransactionlog",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPaymentTransactionLog");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.paymenttransactionlog",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("paymentTransactionLog", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeQ.add(searchtext);
							    								...
							    								if (searchby.equalsIgnoreCase("noVoucherNumber")){
					vLikeP.add("noVoucherNumber");
					vLikeQ.add(searchtext);
				}
							    							    								if (searchby.equalsIgnoreCase("currency")){
					vLikeP.add("currency");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = paymentTransactionLogService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = paymentTransactionLogService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("PaymentTransactionLogs",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPaymentTransactionLog" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPaymentTransactionLog" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPaymentTransactionLog" );
	    }
