...
*/
public class ProviderGroupController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProviderGroup res = providerGroupService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.providergroup",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.providergroup",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProviderGroup");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.providergroup",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("providerGroup", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeQ.add(searchtext);
							    							    								...
							    							    								if (searchby.equalsIgnoreCase("providerGroupName")){
					vLikeP.add("providerGroupName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("providerGroupCode")){
					vLikeP.add("providerGroupCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = providerGroupService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = providerGroupService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProviderGroups",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProviderGroup" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProviderGroup" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProviderGroup" );
	    }
