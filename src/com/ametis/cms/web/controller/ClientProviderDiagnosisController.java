...
/**
 * ClientProviderDiagnosis is a servlet controller for client_provider_diagnosis
 * Table. All you have to do is to convert necessary data field to the named
 * parameter
 */
public class ClientProviderDiagnosisController implements Controller
...

{
	...
	private ClientProviderDiagnosisService clientProviderDiagnosisService;
	...
	public void setClientProviderDiagnosisService(
			ClientProviderDiagnosisService clientProviderDiagnosisService) {
		this.clientProviderDiagnosisService = clientProviderDiagnosisService;
	}
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer clientProviderDiagnosisId = WebUtil.getParameterInteger(
					request, "clientProviderDiagnosisId");
			...
			ClientProviderDiagnosis res = clientProviderDiagnosisService
					.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.clientproviderdiagnosis", null,
						"success", Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.clientproviderdiagnosis", null, "fail",
						Locale.getDefault()));

			}
			result = searchPerformed(request, response,
					"searchClientProviderDiagnosis");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			java.io.Serializable pkey = clientProviderDiagnosisId;
			ClientProviderDiagnosis object = clientProviderDiagnosisService
					.get(pkey);

			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.clientproviderdiagnosis", null, "fail",
						Locale.getDefault()));
			}
			/*
			...
			result.addObject("clientProviderDiagnosis", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			if (clientProviderId != null){
				vEqP.add("clientProviderId.clientProviderId");
				vEqQ.add(clientProviderId);
			}
			...
			count = clientProviderDiagnosisService.getTotal(sLikeP, sLikeQ,
					sEqP, sEqQ);
			...
			String required[] = new String[] {
					// foreign affairs
					"ClientProviderDiagnosis.DiagnosisId",
					"ClientProviderDiagnosis.ClientProviderId",
			// foreign affairs end
			};
			collection = clientProviderDiagnosisService.search(sLikeP, sLikeQ,
			...
			if (collection.size() <= 0) {
				...
				collection = clientProviderDiagnosisService.search(sLikeP,
			...
			result.addObject("ClientProviderDiagnosiss", collection);
			
			request.setAttribute("clientProviderId", clientProviderId);
