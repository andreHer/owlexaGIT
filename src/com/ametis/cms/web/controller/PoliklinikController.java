...
*/
public class PoliklinikController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			Poliklinik res = poliklinikService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.poliklinik",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.poliklinik",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPoliklinik");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.poliklinik",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("poliklinik", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeP.add("poliklinikName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("poliklinikCode")){
					vLikeP.add("poliklinikCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = poliklinikService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = poliklinikService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Polikliniks",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPoliklinik" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPoliklinik" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPoliklinik" );
	    }
