...
*/
public class FinanceCoaController implements Controller
...

{
	...
	private FinanceCoaService  financeCoaService;
...
private ActivityLogService logService;
	...
	public void setFinanceCoaService	(FinanceCoaService financeCoaService){
	this.financeCoaService = financeCoaService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			FinanceCoa res = financeCoaService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.financecoa",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.financecoa",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchFinanceCoa");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.financecoa",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("financeCoa", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("coaName")){
					vLikeP.add("coaName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("coaCode")){
					vLikeP.add("coaCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = financeCoaService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = financeCoaService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("FinanceCoas",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailFinanceCoa" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupFinanceCoa" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchFinanceCoa" );
	    }
