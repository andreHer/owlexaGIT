...
*/
public class FormulariumMedicineController implements Controller
...

{
	...
	private FormulariumMedicineService  formulariumMedicineService;
	...
	public void setFormulariumMedicineService	(FormulariumMedicineService formulariumMedicineService){
	this.formulariumMedicineService = formulariumMedicineService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			FormulariumMedicine res = formulariumMedicineService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.formulariummedicine",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.formulariummedicine",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchFormulariumMedicine");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.formulariummedicine",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("formulariumMedicine", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("formulariumName")){
					vLikeP.add("formulariumName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("formulariumCode")){
					vLikeP.add("formulariumCode");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("periode")){
					vLikeP.add("periode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = formulariumMedicineService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = formulariumMedicineService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("FormulariumMedicines",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailFormulariumMedicine" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupFormulariumMedicine" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchFormulariumMedicine" );
	    }
