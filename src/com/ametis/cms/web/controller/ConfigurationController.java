...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed (HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ModelAndView result = null;
 	    ...
 	    try {
									Integer configurationId = WebUtil.getParameterInteger (request,"configurationId");
			...
			*/
			...
			Configuration res = configurationService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.configuration",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.configuration",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchConfiguration");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.configuration",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("configuration", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = configurationService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = configurationService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Configurations",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailConfiguration" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupConfiguration" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchConfiguration" );
	    }
