...
*/
public class PolicyDocumentController implements Controller
...

{
	...
	public void setPolicyDocumentService	(PolicyDocumentService policyDocumentService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			PolicyDocument res = policyDocumentService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.policydocument",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.policydocument",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPolicyDocument");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.policydocument",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("policyDocument", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    							    								...
			    							    							    								if (searchby.equalsIgnoreCase("documentName")){
					vLikeP.add("documentName");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = policyDocumentService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = policyDocumentService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("PolicyDocuments",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPolicyDocument" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPolicyDocument" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPolicyDocument" );
	    }
