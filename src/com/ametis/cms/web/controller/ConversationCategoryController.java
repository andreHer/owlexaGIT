...
*/
public class ConversationCategoryController implements Controller
...
private ActivityLogService logService;
	...
	public void setConversationCategoryService	(ConversationCategoryService conversationCategoryService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECONVERSATIONCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECONVERSATIONCATEGORY access");
				return errorResult;
				
			}
			ConversationCategory res = conversationCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.conversationcategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.conversationcategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchConversationCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.conversationcategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("conversationCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("conversationCategoryName")){
					vLikeP.add("conversationCategoryName");
					...
					vLikeQ.add(searchtext);
							    							    								...
							    							    								if (searchby.equalsIgnoreCase("updatedBy")){
					vLikeP.add("updatedBy");
					vLikeQ.add(searchtext);
				}
			...
			count = conversationCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = conversationCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ConversationCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailConversationCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupConversationCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchConversationCategory" );
	    }
