...
import com.ametis.cms.service.FormulariumService;
...
*/
public class FormulariumController implements Controller
...

{
	...
	private FormulariumService  formulariumService;
	...
	public void setFormulariumService	(FormulariumService formulariumService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			Formularium res = formulariumService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.formularium",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.formularium",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchFormularium");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.formularium",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("formularium", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = formulariumService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = formulariumService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Formulariums",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailFormularium" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupFormularium" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchFormularium" );
	    }
