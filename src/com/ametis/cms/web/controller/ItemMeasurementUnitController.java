...
/**
 * ItemMeasurementUnit is a servlet controller for item_measurement_unit Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class ItemMeasurementUnitController implements Controller
...
private ActivityLogService logService;
	...
	public void setItemMeasurementUnitService	(ItemMeasurementUnitService itemMeasurementUnitService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEITEMMEASUREMENTUNIT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEITEMMEASUREMENTUNIT access");
				return errorResult;
				
			}
			ItemMeasurementUnit res = itemMeasurementUnitService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.itemmeasurementunit",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.itemmeasurementunit",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchItemMeasurementUnit");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.itemmeasurementunit",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("itemMeasurementUnit", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = itemMeasurementUnitService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"ItemMeasurementUnit.ItemCategoryId",
				"ItemMeasurementUnit.MeasurementUnitId",
		// foreign affairs end
	};
		    collection = itemMeasurementUnitService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ItemMeasurementUnits",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailItemMeasurementUnit" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupItemMeasurementUnit" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchItemMeasurementUnit" );
	    }
