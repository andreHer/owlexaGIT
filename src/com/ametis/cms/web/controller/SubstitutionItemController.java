...
*/
public class SubstitutionItemController implements Controller
...

{
	...
	private SubstitutionItemService  substitutionItemService;
...
private ActivityLogService logService;
	...
	public void setSubstitutionItemService	(SubstitutionItemService substitutionItemService){
	this.substitutionItemService = substitutionItemService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETESUBSTITUTIONITEM");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETESUBSTITUTIONITEM access");
				return errorResult;
				
			}
			SubstitutionItem res = substitutionItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.substitutionitem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.substitutionitem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchSubstitutionItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.substitutionitem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("substitutionItem", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = substitutionItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"SubstitutionItem.CostContainmentId",
		// foreign affairs end
	};
		    collection = substitutionItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("SubstitutionItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailSubstitutionItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupSubstitutionItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchSubstitutionItem" );
	    }
