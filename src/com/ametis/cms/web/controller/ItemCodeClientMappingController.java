...
*/
public class ItemCodeClientMappingController implements Controller
...
// extends- 
{
	...
	public void setItemCodeClientMappingService	(ItemCodeClientMappingService itemCodeClientMappingService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ItemCodeClientMapping res = itemCodeClientMappingService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.itemcodeclientmapping",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.itemcodeclientmapping",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchItemCodeClientMapping");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.itemcodeclientmapping",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("itemCodeClientMapping", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    							    								...
			    							    							    								if (searchby.equalsIgnoreCase("clientItemCode")){
					vLikeP.add("clientItemCode");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = itemCodeClientMappingService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = itemCodeClientMappingService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ItemCodeClientMappings",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailItemCodeClientMapping" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupItemCodeClientMapping" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchItemCodeClientMapping" );
	    }
