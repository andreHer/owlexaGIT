...
/**
 * ActivityLog is a servlet controller for activity_log Table. All you have to
...

{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer activityLogId = WebUtil.getParameterInteger(request,
					"activityLogId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETELOG");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETELOG access");
				return errorResult;
				
			}
			ActivityLog res = activityLogService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.activitylog", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.activitylog", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchActivityLog");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILLOG");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILLOG access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.activitylog", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("activityLog", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHLOG");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHLOG access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("action")) {
						vLikeP.add("action");
						vLikeQ.add(searchtext);
					}
			...
			count = activityLogService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = activityLogService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = activityLogService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("ActivityLogs", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailActivityLog");
			} else if (navigation.equalsIgnoreCase("delete")) {
				result = deletePerformed(request, response);
			} else if (navigation.equalsIgnoreCase("search")
					|| navigation.equals("gosearch")) {
				result = searchPerformed(request, response, "searchActivityLog");
				breadcrumb = "<a href=\"activitylog\" class=\"linkbreadcrumb\">Search Activity Log</a>";
			} else if (navigation.equalsIgnoreCase("lookup")
					|| navigation.equals("golookup")) {
				result = searchPerformed(request, response, "lookupActivityLog");
			} else {
				result = searchPerformed(request, response, "searchActivityLog");
				breadcrumb = "<a href=\"activitylog\" class=\"linkbreadcrumb\">Search Activity Log</a>";
			}
