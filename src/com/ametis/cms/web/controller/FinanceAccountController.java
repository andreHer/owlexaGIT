...
*/
public class FinanceAccountController implements Controller
...
private ActivityLogService logService;
	...
	public void setFinanceAccountService	(FinanceAccountService financeAccountService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			FinanceAccount res = financeAccountService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.financeaccount",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.financeaccount",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchFinanceAccount");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.financeaccount",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("financeAccount", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = financeAccountService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = financeAccountService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("FinanceAccounts",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailFinanceAccount" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupFinanceAccount" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchFinanceAccount" );
	    }
