...
*/
public class QuotationProductController implements Controller
...
private ActivityLogService logService;
	...
	public void setQuotationProductService	(QuotationProductService quotationProductService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			QuotationProduct res = quotationProductService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.quotationproduct",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.quotationproduct",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchQuotationProduct");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.quotationproduct",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("quotationProduct", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (quotationId != null){
		    	vEqP.add("quotationId.quotationId");
		    	vEqQ.add(quotationId);
		    }
			...
			count = quotationProductService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"QuotationProduct.ProductId",
				"QuotationProduct.QuotationId",
		// foreign affairs end
	};
		    collection = quotationProductService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("QuotationProducts",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailQuotationProduct" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupQuotationProduct" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchQuotationProduct" );
	    }
