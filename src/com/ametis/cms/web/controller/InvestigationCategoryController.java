...
*/
public class InvestigationCategoryController implements Controller
...
// extends- 
{
	...
	private InvestigationCategoryService  investigationCategoryService;
...
private ActivityLogService logService;
	...
	public void setInvestigationCategoryService	(InvestigationCategoryService investigationCategoryService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEINVESTIGATIONCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEINVESTIGATIONCATEGORY access");
				return errorResult;
				
			}
			InvestigationCategory res = investigationCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.investigationcategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.investigationcategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchInvestigationCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.investigationcategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("investigationCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("investigationCategoryName")){
					vLikeP.add("investigationCategoryName");
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = investigationCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = investigationCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("InvestigationCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailInvestigationCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupInvestigationCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchInvestigationCategory" );
	    }
