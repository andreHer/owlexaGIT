...
*/
public class ProviderTypeController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProviderType res = providerTypeService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.providertype",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.providertype",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProviderType");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.providertype",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("providerType", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("providerTypeName")){
					vLikeP.add("providerTypeName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("providerTypeCode")){
					vLikeP.add("providerTypeCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = providerTypeService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = providerTypeService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProviderTypes",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProviderType" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProviderType" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProviderType" );
	    }
