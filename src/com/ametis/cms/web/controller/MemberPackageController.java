...
*/
public class MemberPackageController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEMEMBERPACKAGE");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEMEMBERPACKAGE access");
				return errorResult;
				
			}
			MemberPackage res = memberPackageService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.memberpackage",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.memberpackage",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchMemberPackage");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.memberpackage",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("memberPackage", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = memberPackageService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"MemberPackage.MemberPackageStatus",
				"MemberPackage.MemberId",
				"MemberPackage.PackageId",
		// foreign affairs end
	};
		    collection = memberPackageService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("MemberPackages",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailMemberPackage" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupMemberPackage" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchMemberPackage" );
	    }
