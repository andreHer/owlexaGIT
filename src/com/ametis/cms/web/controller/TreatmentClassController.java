...
*/
public class TreatmentClassController implements Controller
...
// extends- 
{
	...
	private TreatmentClassService  treatmentClassService;
	...
	public void setTreatmentClassService	(TreatmentClassService treatmentClassService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			TreatmentClass res = treatmentClassService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.treatmentclass",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.treatmentclass",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchTreatmentClass");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.treatmentclass",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("treatmentClass", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("treatmentClassName")){
					vLikeP.add("treatmentClassName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("treatmentClassCode")){
					vLikeP.add("treatmentClassCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = treatmentClassService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = treatmentClassService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("TreatmentClasss",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailTreatmentClass" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupTreatmentClass" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchTreatmentClass" );
	    }
