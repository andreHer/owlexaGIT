...
/**
 * Quotation is a servlet controller for quotation Table. All you have to do is
 * to convert necessary data field to the named parameter
 */
public class QuotationController implements Controller
...
// extends-
{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			Quotation res = quotationService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.quotation", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.quotation", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchQuotation");
		} catch (Exception e) {
	...
	public ModelAndView calculatePerformed(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = null;
		...
		try {
			...
			Quotation res = quotationService.get(pkey);

			if (res != null) {				
				System.out.println("QUOTATION RECALCULATE");
				Quotation q = quotationService.calculate(res, user);
			...
			result = detailPerformed(request, response, "detailQuotation");
		} catch (Exception e) {
			// nanti kalo sudah OK codenya e.printStackTrace nya di hapus saja
			e.printStackTrace();
			request.setAttribute("alert", alertProperties.getMessage(
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.quotation", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("quotation", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("quotationNo")) {
						vLikeP.add("quotationNo");
					...
					if (searchby.equalsIgnoreCase("requestedBy")) {
						vLikeP.add("requestedBy");
						vLikeQ.add(searchtext);
					}
				}
			...
			count = quotationService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			String required[] = new String[] {
					// foreign affairs
					"Quotation.BrokerId", "Quotation.ClientId",
					"Quotation.MemberGroupId", "Quotation.CurrencyId",
					"Quotation.BranchId", "Quotation.BrokerId",
			// foreign affairs end
			};
			collection = quotationService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = quotationService.search(sLikeP, sLikeQ, sEqP,
			...
			System.out.println("QUOTATION : " + collection);
			result.addObject("Quotations", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
			...
			} else if (navigation.equalsIgnoreCase("calculate")) {
				result = calculatePerformed(request, response);
				...
				result = searchPerformed(request, response, "lookupQuotation");
