...
/**
 * CaseHistory is a servlet controller for case_history Table. All you have to
 * do is to convert necessary data field to the named parameter
 */
public class CaseHistoryController implements Controller
...
// extends-
{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer caseHistoryId = WebUtil.getParameterInteger(request,
					"caseHistoryId");
			...
			CaseHistory res = caseHistoryService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.casehistory", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.casehistory", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchCaseHistory");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.casehistory", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("caseHistory", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("beforeActionData")) {
						vLikeP.add("beforeActionData");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("afterActionData")) {
						vLikeP.add("afterActionData");
						vLikeQ.add(searchtext);
					}
			...
			count = caseHistoryService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			// foreign affairs
					"CaseHistory.AfterStatus", "CaseHistory.BeforeStatus",
			// foreign affairs end
			};
			collection = caseHistoryService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = caseHistoryService.search(sLikeP, sLikeQ, sEqP,
...
//					System.out.println("member group object : "+caseObject.getAddress());
			...
			request.setAttribute("myCase", caseObject);
			...
			result.addObject("CaseHistorys", collection);
	...
	public ModelAndView downloadPerformed(HttpServletRequest request,
		...
		try {
			...
			HSSFWorkbook workbook = CaseReportDownloader.downloadCaseHistoryMonitoring(collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailCaseHistory");
			} else if (navigation.equalsIgnoreCase("delete")) {
				result = deletePerformed(request, response);
			} else if (navigation.equalsIgnoreCase("search")
					|| navigation.equals("gosearch")) {
				result = searchPerformed(request, response, "searchCaseHistory");
				breadcrumb = "<a href=\"case?navigation=detail&caseId="+caseId+"\" class=\"linkbreadcrumb\">Detail Case</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Case History";
			} else if (navigation.equalsIgnoreCase("lookup")
					|| navigation.equals("golookup")) {
				result = searchPerformed(request, response, "lookupCaseHistory");
			}
			else if (navigation.equalsIgnoreCase("download")){				
				result = downloadPerformed(request, response, "searchCaseHistory");
				breadcrumb = "<a href=\"case?navigation=detail&caseId="+caseId+"\" class=\"linkbreadcrumb\">Detail Case</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Case History";
			}
			else {
				result = searchPerformed(request, response, "searchCaseHistory");
