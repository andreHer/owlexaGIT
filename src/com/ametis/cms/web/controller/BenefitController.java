...
/**
 * Benefit is a servlet controller for benefit Table. All you have to do is to
 * convert necessary data field to the named parameter
 */
public class BenefitController implements Controller
...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer benefitId = WebUtil.getParameterInteger(request,
					"benefitId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEBENEFIT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEBENEFIT access");
				return errorResult;
				
			}
			Benefit res = benefitService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.benefit", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.benefit", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchBenefit");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILBENEFIT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILBENEFIT access");
				return errorResult;
			...
			if (object == null) {
				request
						.setAttribute("alert", alertProperties.getMessage(
								"not.found.benefit", null, "fail", Locale
										.getDefault()));
			}
			/*
			...
			result.addObject("benefit", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHBENEFIT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHBENEFIT access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("benefitName")) {
						vLikeP.add("benefitName");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("benefitDescription")) {
						vLikeP.add("benefitDescription");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("benefitMeasurementUnit")) {
						vLikeP.add("benefitMeasurementUnit");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("benefitOccuranceUnit")) {
						vLikeP.add("benefitOccuranceUnit");
						vLikeQ.add(searchtext);
					}
			...
			count = benefitService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = benefitService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			result.addObject("Benefits", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailBenefit");
				...
				result = searchPerformed(request, response, "lookupBenefit");
			} else {
				result = searchPerformed(request, response, "searchBenefit");
			}
		} catch (Exception e) {
