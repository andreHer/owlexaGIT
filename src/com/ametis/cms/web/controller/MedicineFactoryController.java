...
*/
public class MedicineFactoryController implements Controller
...

{
	...
	public void setMedicineFactoryService	(MedicineFactoryService medicineFactoryService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			MedicineFactory res = medicineFactoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.medicinefactory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.medicinefactory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchMedicineFactory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.medicinefactory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("medicineFactory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("medicineFactoryName")){
					vLikeP.add("medicineFactoryName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("medicineFactoryCode")){
					vLikeP.add("medicineFactoryCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = medicineFactoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = medicineFactoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("MedicineFactorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailMedicineFactory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupMedicineFactory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchMedicineFactory" );
	    }
