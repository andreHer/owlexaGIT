...
*/
public class RefProvinceController implements Controller
...
private ActivityLogService logService;
	...
	return this.refProvinceService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			RefProvince res = refProvinceService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.refprovince",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.refprovince",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchRefProvince");
		}
		catch (Exception e){
	    ...
	    try {
			...
			*/
			...
			result = new ModelAndView (location);
						java.io.Serializable pkey = id;
				        RefProvince object = refProvinceService.get (pkey);

			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.refprovince",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("refProvince", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    							    								...
			    							    							    								if (searchby.equalsIgnoreCase("provinceName")){
					vLikeP.add("provinceName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("provinceCode")){
					vLikeP.add("provinceCode");
					vLikeQ.add(searchtext);
				}
					...
					vLikeQ.add(searchtext);
							    								...
							    								if (searchby.equalsIgnoreCase("longitude")){
					vLikeP.add("longitude");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("latitude")){
					vLikeP.add("latitude");
					vLikeQ.add(searchtext);
				}
  		        }
			...
			count = refProvinceService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = refProvinceService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("RefProvinces",collection);
	...
	public ModelAndView lookupJsonPerformed(HttpServletRequest request,
		...
		try {
			...
			System.out.println("lookup json province dipanggil");
			
			String q = WebUtil.getParameterString(request, "q", "");
			Integer countryId = WebUtil.getParameterInteger(request, "countryId");
			String countryName = WebUtil.getParameterString(request, "countryName", "");
			
			System.out.println("q : " + q);
			System.out.println("countryId : " + countryId);
			System.out.println("countryName : " + countryName);
			
			Collection<RefProvince> cities = null; 
				
			if (countryId == null && countryName.equalsIgnoreCase("")){
				cities = refProvinceService.searchProvince(q.toLowerCase());
			}
			else if (!countryName.equalsIgnoreCase("")){
				cities = refProvinceService.searchProvince(q.toLowerCase(), countryName) ;
			}
			else if (countryId != null) {
				cities = refProvinceService.searchProvince(q.toLowerCase(),countryId);
			}
			
			
			result.addObject("Provinces", cities);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailRefProvince" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupRefProvince" );
	    }
	    else if (navigation.equalsIgnoreCase("lookupjson")){
	    	result = lookupJsonPerformed(request, response, "lookupJsonRefProvince");
	    }
	    else {
	    	result = searchPerformed (request, response, "searchRefProvince" );
	    }
