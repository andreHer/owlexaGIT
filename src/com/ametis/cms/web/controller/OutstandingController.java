...
/**
 * Outstanding is a servlet controller for outstanding Table. All you have to do
 * is to convert necessary data field to the named parameter
 */
public class OutstandingController implements Controller
...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEOUTSTANDING");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEOUTSTANDING access");
				return errorResult;
				
			}
			Outstanding res = outstandingService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.outstanding", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.outstanding", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchOutstanding");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			java.io.Serializable pkey = outstandingId;
			String[] required = {"Outstanding.BatchClaimId","Outstanding.BatchClaimId.Claims"};
			
			Outstanding object = outstandingService.get(pkey ,required);

			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.outstanding", null, "fail", Locale
								.getDefault()));
			}
			...
			result.addObject("outstanding", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			Integer searchstatus = WebUtil.getParameterInteger(request,"searchstatus");
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("batchNumber")) {
						vLikeP.add("batchClaimId.batchClaimNumber");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("outstandingValue")) {
						vEqP.add("outstandingValue");
						vEqQ.add(searchtext);
					}
				}
				if (searchstatus != null){
					vEqP.add("outstandingStatus.paymentStatusId");
			...
			count = outstandingService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			// foreign affairs
					"Outstanding.ClientId", "Outstanding.BatchClaimId",
			// foreign affairs end
			};
			collection = outstandingService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = outstandingService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("Outstandings", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailOutstanding");
				...
				result = searchPerformed(request, response, "lookupOutstanding");
			} else {
				result = searchPerformed(request, response, "searchOutstanding");
			}
		} catch (Exception e) {
