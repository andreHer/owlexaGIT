...
*/
public class TarifTypeController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			TarifType res = tarifTypeService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.tariftype",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.tariftype",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchTarifType");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.tariftype",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("tarifType", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("tarifTypeName")){
					vLikeP.add("tarifTypeName");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = tarifTypeService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = tarifTypeService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("TarifTypes",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailTarifType" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupTarifType" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchTarifType" );
	    }
