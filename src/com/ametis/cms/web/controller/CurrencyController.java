...
/**
 * Currency is a servlet controller for currency Table.
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECURRENCY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECURRENCY access");
				return errorResult;
				
			}
			com.ametis.cms.datamodel.Currency res = currencyService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.currency",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.currency",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchCurrency");
		}
		catch (Exception e){
	    ...
	    try {
			...
			*/
			...
			result = new ModelAndView (location);
						java.io.Serializable pkey = currencyId;
				        com.ametis.cms.datamodel.Currency object = currencyService.get (pkey);

			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.currency",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("currency", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("currencyCode")){
					vLikeP.add("currencyCode");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("currencyName")){
					vLikeP.add("currencyName");
					vLikeQ.add(searchtext);
				}
			...
			count = currencyService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = currencyService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Currencys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailCurrency" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupCurrency" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchCurrency" );
	    }
