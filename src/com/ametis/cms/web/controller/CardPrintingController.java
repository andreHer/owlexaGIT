...
/**
 * CardPrinting is a servlet controller for card_printing Table. All you have to
 * do is to convert necessary data field to the named parameter
 */
public class CardPrintingController implements Controller
...
// extends-
{
	...
	public void setCardPrintingService(CardPrintingService cardPrintingService) {
	...
	public ModelAndView deletePerformed(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			CardPrinting res = cardPrintingService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage("success.delete.cardprinting", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert",
						alertProperties.getMessage("fail.delete.cardprinting", null, "fail", Locale.getDefault()));

			}
			result = searchPerformed(request, response, "searchCardPrinting");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request, HttpServletResponse response, String location)
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert",
						alertProperties.getMessage("not.found.cardprinting", null, "fail", Locale.getDefault()));
			}
			/*
			...
			result.addObject("cardPrinting", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("printingNumber")) {
						vLikeP.add("printingNumber");
			...
			count = cardPrintingService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			String required[] = new String[] {
					// foreign affairs
					"CardPrinting.ClientId",
					// foreign affairs end
			};
			collection = cardPrintingService.search(sLikeP, sLikeQ, sEqP, sEqQ, required, rowsetint,
					countSet.intValue());
			...
			result.addObject("CardPrintings", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailCardPrinting");
				...
				result = searchPerformed(request, response, "lookupCardPrinting");
			} else {
				result = searchPerformed(request, response, "searchCardPrinting");
			}
		} catch (Exception e) {
