...
*/
public class ProviderRollHistoryController implements Controller
...
// extends- 
{
	...
	private ProviderRollHistoryService  providerRollHistoryService;
	...
	public void setProviderRollHistoryService	(ProviderRollHistoryService providerRollHistoryService){
	this.providerRollHistoryService = providerRollHistoryService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProviderRollHistory res = providerRollHistoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.providerrollhistory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.providerrollhistory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProviderRollHistory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.providerrollhistory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("providerRollHistory", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    String[] searchByDate = { "deliveryDate" };
			...
			if(minimumDate != null && maximumDate != null ) {
				if (minimumDate.toString().equals("1970-01-01") && maximumDate.toString().equals("1970-01-01")){
					count = providerRollHistoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
				}
				else {
					count = providerRollHistoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ, searchByDate, minimumDateArray, maximumDateArray);
				}
			}else{
				count = providerRollHistoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
			}
			...
			if (minimumDate != null && maximumDate != null &&
				...
				collection = providerRollHistoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
						searchByDate, minimumDateArray, maximumDateArray,required,rowsetint,countSet.intValue());
			} else {
				collection = providerRollHistoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,required,rowsetint,countSet.intValue());
			}
		    ...
		    */
		    ...
		    result.addObject("ProviderRollHistorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProviderRollHistory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProviderRollHistory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProviderRollHistory" );
	    }
