...
/**
 * Clausul is a servlet controller for clausul Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class ClausulController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECLAUSUL");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECLAUSUL access");
				return errorResult;
				
			}
			Clausul res = clausulService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.clausul",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.clausul",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchClausul");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.clausul",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("clausul", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeP.add("clausulName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("clausulCode")){
					vLikeP.add("clausulCode");
					vLikeQ.add(searchtext);
				}
			...
			count = clausulService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"Clausul.ClausulCategoryId",
		// foreign affairs end
	};
		    collection = clausulService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Clausuls",collection);
	...
	public ModelAndView lookupJsonPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ModelAndView result = null;
		try {
		    result = new ModelAndView (location);

		    String q = request.getParameter("q");
		    String[] likeParam = {"clausulName"};
		    String[] eqParam = {"deletedStatus"};
		    System.out.println("PARAM : "+q.toLowerCase());
		    Object[] likeValue = {q};
		    Object[] eqValue = {Integer.valueOf(0)};
		    
		    Collection<Clausul> clausulList = clausulService.search(likeParam,likeValue,eqParam,eqValue, true, "clausulName",0,15);
		    
		    result.addObject("Clausuls", clausulList);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailClausul" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupClausul" );
	    }
	    else if (navigation.equalsIgnoreCase("lookupjson")){
	    	result = lookupJsonPerformed(request, response, "lookupClausulJson");
	    }
	    else {
	    	result = searchPerformed (request, response, "searchClausul" );
	    }
