...
*/
public class MemberDiagnosisController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEMEMBERDIAGNOSIS");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEMEMBERDIAGNOSIS access");
				return errorResult;
				
			}
			MemberDiagnosis res = memberDiagnosisService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.memberdiagnosis",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.memberdiagnosis",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchMemberDiagnosis");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.memberdiagnosis",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("memberDiagnosis", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = memberDiagnosisService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				...
				"MemberDiagnosis.MemberId",
		// foreign affairs end
	};
		    collection = memberDiagnosisService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("MemberDiagnosiss",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailMemberDiagnosis" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupMemberDiagnosis" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchMemberDiagnosis" );
	    }
