...
/**
 * Dependent is a servlet controller for dependent Table. All you have to do is
 * to convert necessary data field to the named parameter
 */
public class DependentController implements Controller
...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer dependentId = WebUtil.getParameterInteger(request,
					"dependentId");
...
ActionUser user= securityService.getActionUser(request); 
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEDEPENDENT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEDEPENDENT access");
				return errorResult;
				
			}
			
			Dependent res = dependentService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.dependent", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.dependent", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchDependent");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.dependent", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("dependent", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("dependantName")) {
						vLikeP.add("dependantName");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("dependantNumber")) {
						vLikeP.add("dependantNumber");
						vLikeQ.add(searchtext);
					}
			...
			count = dependentService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			// foreign affairs
					"Dependent.Status", "Dependent.MemberId",
					"Dependent.RelationshipId",
			// foreign affairs end
			};
			collection = dependentService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = dependentService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("Dependents", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailDependent");
				...
				result = searchPerformed(request, response, "lookupDependent");
			} else {
				result = searchPerformed(request, response, "searchDependent");
			}
		} catch (Exception e) {
