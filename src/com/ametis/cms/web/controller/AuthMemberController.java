...
/**
 * AuthMember is a servlet controller for auth_member Table. All you have to do
 * is to convert necessary data field to the named parameter
 */
public class AuthMemberController implements Controller
...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer authMemberId = WebUtil.getParameterInteger(request,
					"authMemberId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "AUTHMEMBER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for AUTHMEMBER access");
				return errorResult;
				
			}
			AuthMember res = authMemberService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.authmember", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.authmember", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchAuthMember");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILAUTHMEMBER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILAUTHMEMBER access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.authmember", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("authMember", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHAUTHMEMBER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHAUTHMEMBER access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("password")) {
						vLikeP.add("password");
						vLikeQ.add(searchtext);
					}
					...
					if (searchby.equalsIgnoreCase("activationCode")) {
						vLikeP.add("activationCode");
						vLikeQ.add(searchtext);
					}
				}
			...
			count = authMemberService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = authMemberService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = authMemberService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("AuthMembers", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailAuthMember");
				...
				result = searchPerformed(request, response, "lookupAuthMember");
			} else {
				result = searchPerformed(request, response, "searchAuthMember");
			}
		} catch (Exception e) {
