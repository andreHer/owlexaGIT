...
/**
 * BillingItem is a servlet controller for billing_item Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class BillingItemController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			BillingItem res = billingItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.billingitem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.billingitem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchBillingItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.billingitem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("billingItem", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = billingItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"BillingItem.MemberImportId",
				"BillingItem.ClaimId",
				"BillingItem.ClientId",
				"BillingItem.MemberId",
				"BillingItem.PolicyId",
		// foreign affairs end
	};
		    collection = billingItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("BillingItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailBillingItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupBillingItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchBillingItem" );
	    }
