...
/**
 * BenefitCategory is a servlet controller for benefit_category Table. All you
 * have to do is to convert necessary data field to the named parameter
 */
public class BenefitCategoryController implements Controller
...
private ActivityLogService logService;
	...
	public void setBenefitCategoryService(
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer benefitCategoryId = WebUtil.getParameterInteger(request,
					"benefitCategoryId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEBENEFITCAT");
			...
			BenefitCategory res = benefitCategoryService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.benefitcategory", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.benefitcategory", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchBenefitCategory");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILBENEFITCAT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILBENEFITCAT access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.benefitcategory", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("benefitCategory", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHBENEFITCAT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHBENEFITCAT access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("benefitCategoryName")) {
						vLikeP.add("benefitCategoryName");
			...
			count = benefitCategoryService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = benefitCategoryService.search(sLikeP, sLikeQ, sEqP,
			...
			if (collection.size() <= 0) {
				...
				collection = benefitCategoryService.search(sLikeP, sLikeQ,
			...
			result.addObject("BenefitCategorys", collection);
