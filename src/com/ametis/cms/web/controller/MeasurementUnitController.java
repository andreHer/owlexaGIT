...
*/
public class MeasurementUnitController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEMEASUREMENT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEMEASUREMENT access");
				return errorResult;
				
			}
			MeasurementUnit res = measurementUnitService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.measurementunit",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.measurementunit",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchMeasurementUnit");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.measurementunit",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("measurementUnit", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("measurementUnitName")){
					vLikeP.add("measurementUnitName");
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = measurementUnitService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = measurementUnitService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("MeasurementUnits",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailMeasurementUnit" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupMeasurementUnit" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchMeasurementUnit" );
	    }
