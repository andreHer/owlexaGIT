...
*/
public class InvestigationController implements Controller
...
private ActivityLogService logService;
	...
	public void setInvestigationService	(InvestigationService investigationService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEINVESTIGATION");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEINVESTIGATION access");
				return errorResult;
				
			}	
			Investigation res = investigationService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.investigation",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.investigation",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchInvestigation");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.investigation",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("investigation", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = investigationService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = investigationService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Investigations",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailInvestigation" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupInvestigation" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchInvestigation" );
	    }
