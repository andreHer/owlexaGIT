...
*/
public class ProductDiagnosisClausulController implements Controller
...

{
	...
	private ProductDiagnosisClausulService  productDiagnosisClausulService;
	...
	public void setProductDiagnosisClausulService	(ProductDiagnosisClausulService productDiagnosisClausulService){
	this.productDiagnosisClausulService = productDiagnosisClausulService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProductDiagnosisClausul res = productDiagnosisClausulService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.productdiagnosisclausul",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.productdiagnosisclausul",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProductDiagnosisClausul");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.productdiagnosisclausul",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("productDiagnosisClausul", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = productDiagnosisClausulService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = productDiagnosisClausulService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProductDiagnosisClausuls",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProductDiagnosisClausul" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProductDiagnosisClausul" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProductDiagnosisClausul" );
	    }
