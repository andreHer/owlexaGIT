...
*/
public class MemberLimitLayerController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			MemberLimitLayer res = memberLimitLayerService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.memberlimitlayer",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.memberlimitlayer",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchMemberLimitLayer");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.memberlimitlayer",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("memberLimitLayer", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = memberLimitLayerService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"MemberLimitLayer.ProductLimitLayerId",
				"MemberLimitLayer.MemberId",
				"MemberLimitLayer.ProductId",
		// foreign affairs end
	};
		    collection = memberLimitLayerService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("MemberLimitLayers",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailMemberLimitLayer" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupMemberLimitLayer" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchMemberLimitLayer" );
	    }
