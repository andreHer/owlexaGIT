...
*/
public class PolicyProductController implements Controller
...
private ActivityLogService logService;
	...
	public void setPolicyProductService	(PolicyProductService policyProductService){
	...
	public ModelAndView deletePerformed (HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ModelAndView result = null;
 	    ...
 	    try {
									Long policyProductId = WebUtil.getParameterLong (request, "policyProductId");
			...
			*/
			...
			PolicyProduct res = policyProductService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.policyproduct",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.policyproduct",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPolicyProduct");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.policyproduct",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("policyProduct", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
			...
			count = policyProductService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"PolicyProduct.PolicyId",
				"PolicyProduct.ProductId",
		// foreign affairs end
	};
		    collection = policyProductService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    result.addObject("PolicyProducts",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPolicyProduct" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPolicyProduct" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPolicyProduct" );
	    }
