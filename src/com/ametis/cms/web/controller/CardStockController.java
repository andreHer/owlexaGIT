...
/**
 * CardStock is a servlet controller for card_stock Table. All you have to do is
 * to convert necessary data field to the named parameter
 */
public class CardStockController implements Controller
...
// extends-
{
	...
	public void setCardStockService(CardStockService cardStockService) {
		this.cardStockService = cardStockService;
	}
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			CardStock res = cardStockService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.cardstock", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.cardstock", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchCardStock");
		} catch (Exception e) {
	...
	public ModelAndView approvePerformed(HttpServletRequest request,
		...
		try {
			...
			boolean res = cardStockService.approve(pkey, user);

			if (res) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.approve.cardstock", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.approve.cardstock", null, "fail", Locale
								.getDefault()));
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.cardstock", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("cardStock", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup") || navigation.equals("listclient")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("cardStockBatchNumber")) {
						vLikeP.add("cardStockBatchNumber");
			...
			count = cardStockService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = cardStockService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = cardStockService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("CardStocks", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailCardStock");
				...
				result = searchPerformed(request, response, "lookupCardStock");
			} else {
				result = searchPerformed(request, response, "searchCardStock");
				String clientId = request.getParameter("clientId");
				breadcrumb = "<a href=\"client?navigation=detail&clientId="+clientId+"\" class=\"linkbreadcrumb\">Detail Client</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Client Card Stock";
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
