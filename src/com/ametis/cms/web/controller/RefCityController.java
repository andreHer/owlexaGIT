...
/**
 * RefCity is a servlet controller for ref_city Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class RefCityController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			RefCity res = refCityService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.refcity",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.refcity",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchRefCity");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.refcity",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("refCity", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("cityName")){
					vLikeP.add("cityName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("cityCode")){
					vLikeP.add("cityCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = refCityService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = refCityService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("RefCitys",collection);
	...
	public ModelAndView lookupJsonPerformed(HttpServletRequest request,
		...
		try {
			...
			System.out.println("lookup json city dipanggil");
			
			String q = WebUtil.getParameterString(request, "q", "");
			Integer provinceId = WebUtil.getParameterInteger(request, "provinceId");
			String provinceName = WebUtil.getParameterString(request, "provinceName", "");
			
			System.out.println("q : " + q);
			System.out.println("provinceId : " + provinceId);
			System.out.println("provinceName : " + provinceName);
			
			Collection<RefCity> cities = null; 
				
			if (provinceId == null && provinceName.equalsIgnoreCase("")){
				cities = refCityService.searchCity(q);
			}
			else if (!provinceName.equalsIgnoreCase("")){
				cities = refCityService.searchCity(q.toLowerCase(), provinceName) ;
			}
			else if (provinceId != null) {
				cities = refCityService.searchCity(q.toLowerCase(),provinceId);
			}
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailRefCity" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupRefCity" );
	    }
	    else if (navigation.equalsIgnoreCase("lookupjson")){
	    	result = lookupJsonPerformed(request, response, "lookupJsonRefCity");
	    }
	    else {
	    	result = searchPerformed (request, response, "searchRefCity" );
	    }
