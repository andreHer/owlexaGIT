 ...
 */
public class PolicyCoverageFundController implements Controller
...
// extends-
{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer policyCoverageFundId = WebUtil.getParameterInteger(request,
					"policyCoverageFundId");
			...
			PolicyCoverageFund res = policyCoverageFundService.delete(pkey,
					user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.policycoveragefund", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.policycoveragefund", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response,
					"searchPolicyCoverageFund");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.policycoveragefund", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("policyCoverageFund", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			count = policyCoverageFundService.getTotal(sLikeP, sLikeQ, sEqP,
					sEqQ);
			...
			collection = policyCoverageFundService.search(sLikeP, sLikeQ, sEqP,
			...
			if (collection.size() <= 0) {
				...
				collection = policyCoverageFundService.search(sLikeP, sLikeQ,
			...
			result.addObject("PolicyCoverageFunds", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				...
				result = searchPerformed(request, response,
						"lookupPolicyCoverageFund");
			} else if (navigation.equalsIgnoreCase("listpolicy")) {          
				result = searchPerformed(request, response, "searchPolicyCoverageFund");
				String policyId = request.getParameter("policyId");
				breadcrumb = "<a href=\"case?navigation=listpolicy&policyId="+policyId+"\" class=\"linkbreadcrumb\">Detail Policy</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Policy Coverage Fund";
			} else {
				result = searchPerformed(request, response,
						"searchPolicyCoverageFund");
			}
