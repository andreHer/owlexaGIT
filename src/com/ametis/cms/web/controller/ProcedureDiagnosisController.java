...
*/
public class ProcedureDiagnosisController implements Controller
...
// extends- 
{
	...
	private ProcedureDiagnosisService  procedureDiagnosisService;
	...
	public void setProcedureDiagnosisService	(ProcedureDiagnosisService procedureDiagnosisService){
	this.procedureDiagnosisService = procedureDiagnosisService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProcedureDiagnosis res = procedureDiagnosisService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.procedurediagnosis",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.procedurediagnosis",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProcedureDiagnosis");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.procedurediagnosis",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("procedureDiagnosis", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = procedureDiagnosisService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"ProcedureDiagnosis.DiagnosisId",
				"ProcedureDiagnosis.ProcedureId",
		// foreign affairs end
	};
		    collection = procedureDiagnosisService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProcedureDiagnosiss",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProcedureDiagnosis" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProcedureDiagnosis" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProcedureDiagnosis" );
	    }
