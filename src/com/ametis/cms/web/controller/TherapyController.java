...

{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			Therapy res = therapyService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.therapy",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.therapy",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchTherapy");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.therapy",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("therapy", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("therapyName")){
					vLikeP.add("therapyName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("therapyCode")){
					vLikeP.add("therapyCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = therapyService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = therapyService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Therapys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailTherapy" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupTherapy" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchTherapy" );
	    }
