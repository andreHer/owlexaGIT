...
*/
public class ContractTypeController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ContractType res = contractTypeService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.contracttype",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.contracttype",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchContractType");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.contracttype",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("contractType", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("contractTypeName")){
					vLikeP.add("contractTypeName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("contractTypeCode")){
					vLikeP.add("contractTypeCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = contractTypeService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = contractTypeService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ContractTypes",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailContractType" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupContractType" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchContractType" );
	    }
