...
*/
public class PolicyMemberController implements Controller
...
private ActivityLogService logService;
	...
	public void setPolicyMemberService	(PolicyMemberService policyMemberService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			PolicyMember res = policyMemberService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.policymember",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.policymember",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPolicyMember");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.policymember",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("policyMember", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = policyMemberService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		...
		// foreign affairs end
		    ...
		    collection = policyMemberService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("PolicyMembers",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPolicyMember" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPolicyMember" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPolicyMember" );
	    }
