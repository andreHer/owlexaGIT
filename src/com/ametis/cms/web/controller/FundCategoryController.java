...
*/
public class FundCategoryController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEFUNDCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEFUNDCATEGORY access");
				return errorResult;
				
			}
			FundCategory res = fundCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.fundcategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.fundcategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchFundCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.fundcategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("fundCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("fundCategoryName")){
					vLikeP.add("fundCategoryName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("fundCategoryCode")){
					vLikeP.add("fundCategoryCode");
					vLikeQ.add(searchtext);
				}
			...
			count = fundCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = fundCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("FundCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailFundCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupFundCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchFundCategory" );
	    }
