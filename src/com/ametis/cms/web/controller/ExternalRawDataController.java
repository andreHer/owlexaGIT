...
/**
 * ExternalRawData is a servlet controller for external_raw_data Table. All you
 * have to do is to convert necessary data field to the named parameter
 */
public class ExternalRawDataController implements Controller
...
// extends-
{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			ExternalRawData res = externalRawDataService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.externalrawdata", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.externalrawdata", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchExternalRawData");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.externalrawdata", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("externalRawData", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			Integer sessionId = WebUtil.getParameterInteger(request,
					"id");
			Integer movementId = WebUtil.getParameterInteger(request,
					"movementId");
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("rawData")) {
						vLikeP.add("rawData");
			...
			if (sessionId != null) {
				vEqP.add("importSessionId.id");
				vEqQ.add(sessionId);
			}

			if (movementId != null) {
				vEqP.add("movementId.id");
				vEqQ.add(movementId);
			}
			...
			count = externalRawDataService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			// foreign affairs
			"ExternalRawData.TemplateId",
			// foreign affairs end
			};
			collection = externalRawDataService.search(sLikeP, sLikeQ, sEqP,
			...
			if (collection.size() <= 0) {
				...
				collection = externalRawDataService.search(sLikeP, sLikeQ,
			...
			result.addObject("ExternalRawDatas", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			...
			} else if (navigation.equalsIgnoreCase("listmovement")) {
				Integer sessionId = WebUtil.getParameterInteger(request,
						"movementId");

				result = searchPerformed(request, response,
						"listMovementRawData");
				result.addObject("movementId", sessionId);
			} else {
				result = searchPerformed(request, response,
						"searchExternalRawData");
			}
