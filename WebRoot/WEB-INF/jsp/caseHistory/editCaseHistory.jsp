...
if (alert != null && !alert.trim().equals("")) {%>
	...
	<%}%>
<form action="casehistory-form" method="POST"  name="form1" id="form_layout">
			<spring:bind path="caseHistoryForm.caseHistoryId">
...
<table class="tabForm" border="0" cellpadding="0" cellspacing="0" width="100%">
	<tbody>
		...
		<tr>
			...
			<tr>
				<td class="dataLabel"> Case Id : </td>				
			

						<td class="dataField">
			<spring:bind path="caseHistoryForm.caseId">
	<input type="text" size="35" name="<c:out value="${status.expression}" />" value="<fmt:formatNumber><c:out value="${status.value}"/></fmt:formatNumber>" maxlength="10">
	    ...
	    <td class="dataField"></td>
			...
			<tr>
				<td class="dataLabel"> History Time : </td>				
			

						<td class="dataField">
			<spring:bind path="caseHistoryForm.historyTime">
	    ...
	    <td class="dataField"></td>
			...
			<tr>
				<td class="dataLabel"> Before Action Data : </td>				
			
	
						<td class="dataField">
				<spring:bind path="caseHistoryForm.beforeActionData">
			...
			<tr>
				<td class="dataLabel"> After Action Data : </td>				
			
	
						<td class="dataField">
				<spring:bind path="caseHistoryForm.afterActionData">
				<textarea  cols="60" rows="8" name="<c:out value="${status.expression}"/>"><c:out value="${status.value}" /></textarea>
					<div id="fredcaption">
						<c:out value="${status.errorMessage}" />
					</div>
			...
			<tr>
				<td class="dataLabel"> Action By : </td>				
						...
						<td class="dataField">
				<spring:bind path="caseHistoryForm.actionBy">
	...
	<tr>
		<td class="dataLabel">Case Status Id : </td>		
		<td class="dataField">
		<spring:bind path="caseHistoryForm.afterStatus">
		<input type="hidden" name="<c:out value="${status.expression}"/>" id="<c:out value="${status.expression}"/>" value="<c:out value="${status.value}" />" >
			<div id="fredcaption">
				<c:out value="${status.errorMessage}" />
			</div>
		</spring:bind>	
			<input type="text" name="afterStatusText" id="afterStatusId" value="" size=35>
	...
	<tr>
		<td class="dataLabel">Case Status Id : </td>		
		<td class="dataField">
		<spring:bind path="caseHistoryForm.beforeStatus">
		<input type="hidden" name="<c:out value="${status.expression}"/>" id="<c:out value="${status.expression}"/>" value="<c:out value="${status.value}" />" >
			<div id="fredcaption">
				<c:out value="${status.errorMessage}" />
			</div>
		</spring:bind>	
			<input type="text" name="beforeStatusText" id="beforeStatusId" value="" size=35>
