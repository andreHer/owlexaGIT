...
<link rel="stylesheet" type="text/css"
	href="scripts/jquery/jquery.multiselect.min.js">
<!-- <link rel="stylesheet" type="text/css" href="css/jquery/autocomplete/jquery.autocomplete.css"/> -->
<link rel="stylesheet" type="text/css"
	href="css/jquery/jquery.multiselect.css" />
<!-- <script type="text/javascript" src="scripts/jquery/jquery-1.4.4.min.js"></script> -->

<!-- <script type="text/javascript" src="scripts/jquery/plugin/jquery.autocomplete.pack.js"></script> -->
<script type="text/javascript"
	src="scripts/jquery/jquery.multiselect.min.js"></script>
...
<%
	...
	try {
		...
		if (request.getAttribute("countSet") != null) {
			countSet = ((Integer) request.getAttribute("viewCountSet"))
					.intValue();
		} else {
...
<%
	...
	if (request.getAttribute("status") != null
			&& request.getAttribute("navigation").equals("searchcase")) {
		navStatus = 1;
	} else if (request.getAttribute("navigation").equals("searchopen")) {
		navStatus = 2;
	}
	session.setAttribute("nav", navStatus);
%>

<form name="form1" action="case" method="POST">
	<input type="hidden" name="navigation" value="gosearch"> <input
		type="hidden" name="arah" value=""> <input type="hidden"
		name="sortcolumn" value="<c:out value="${sortcolumn}" />"> <input
		type="hidden" name="sortorder" value="<c:out value="${sortorder}" />">
	<input type="hidden" name="columntosort"
		value="<c:out value="${columntosort}" />"> <input
		type="hidden" name="index" value="<c:out value="${index}" />">
	<input type="hidden" name="caseId"
		value="<c:out value="${myCase.caseId}" />"> <input
		type="hidden" name="providerId" id="providerId"
		value="<c:out value="${providerId}" />" />
		...
		<tbody>
			<tr>
				<td nowrap="nowrap">
					...
					%>
					<h3>
						<img src="images/h3Arrow.gif" border="0">&nbsp;Search Active
						Case
					</h3> <%
 	} else {
 %>
					<h3>
						<img src="images/h3Arrow.gif" border="0">&nbsp;Search
						Payment
					</h3> <%
 	}
 %>
						...
						<tbody>
							<tr>
								<div id="timeDiv" align="center"
									style="color:#FCECD3;border-width: 4px;border-style: outset;border-color:black;">
									Refresh Page in &nbsp;<span id="time"></span>
								</div>
								...
								<td class="dataLabel" nowrap="nowrap">Search Category:
									&nbsp;&nbsp; <select name="searchby" class="inputbox">
										<option value="caseNumber"
											<c:if test="${searchby eq 'caseNumber'}">selected="true"</c:if>>
											Case Number</option>
										<option value="cardNumber"
											<c:if test="${searchby eq 'cardNumber'}">selected="true"</c:if>>
											Card Number</option>
										<option value="memberName"
											<c:if test="${searchby eq 'memberName'}">selected="true"</c:if>>
											Member Name</option>
										<option value="diagnosisName"
											<c:if test="${searchby eq 'diagnosisName'}">selected="true"</c:if>>
											Diagnosis</option>

								</select>
								</td>
								<!-- modified by aju on 20150925, make it multiple select like non provider user fufufu -->
								<%-- 
              		 ...
              		 --%>
								<td class="dataLabel" nowrap="nowrap">Status:
									...
									<select name="status" id="status" multiple="multiple">
										<%
											String[] statusSearch = (String[]) request
													.getAttribute("statusMulti");
										...
										%>
										<c:set var="status2" value="<%= panjangStatus%>">
										</c:set>
										<!-- 						 	<option value="-1">-- All Status --</option> -->
										<option value="1"
											<c:if test="${status2[2] eq 1 }">selected</c:if>>OPEN</option>
										<option value="9"
											<c:if test="${status2[10] eq 9 }">selected</c:if>>APPROVED</option>
										<option value="4"
											<c:if test="${status2[5] eq 4 }">selected</c:if>>REJECTED</option>
										<option value="5"
											<c:if test="${status2[6] eq 5 }">selected</c:if>>CLOSED</option>
										<option value="10"
											<c:if test="${status2[11] eq 10 }">selected</c:if>>PENDING</option>
										<option value="2"
											<c:if test="${status2[3] eq 2 }">selected</c:if>>PENDING
											DOCUMENT</option>
										<option value="7"
											<c:if test="${status2[8] eq 7 }">selected</c:if>>PENDING
											INVESTIGATION</option>
										<option value="15"
											<c:if test="${status2[16] eq 15 }">selected</c:if>>FINALIZED</option>
										<option value="17"
											<c:if test="${status2[18] eq 17 }">selected</c:if>>REFERED</option>
										<option value="18"
											<c:if test="${status2[19] eq 18 }">selected</c:if>>GREY
											AREA</option>
										<option value="19"
											<c:if test="${status2[20] eq 19 }">selected</c:if>>PRE
											OPEN</option>
										<option value="20"
											<c:if test="${status2[21] eq 20 }">selected</c:if>>PRE
											AUTH</option>
										<option value="21"
											<c:if test="${status2[22] eq 21 }">selected</c:if>>PRE
											REJECT</option>
										<option value="22"
											<c:if test="${status2[23] eq 22 }">selected</c:if>>PRE
											PENDING</option>
										<%
											}
											} else {
										...
										<!-- 							<option value="-1">-- All Status --</option> -->
										<option value="-1"
											<c:if test="${status eq -1 }">selected</c:if>>VOID</option>
										<option value="1" <c:if test="${status eq 1 }">selected</c:if>>OPEN</option>
										<option value="9" <c:if test="${status eq 9 }">selected</c:if>>APPROVED</option>
										<option value="4" <c:if test="${status eq 4 }">selected</c:if>>REJECTED</option>
										<option value="5" <c:if test="${status eq 5 }">selected</c:if>>CLOSED</option>
										<option value="10"
											<c:if test="${status eq 10 }">selected</c:if>>PENDING</option>
										<option value="2" <c:if test="${status eq 2 }">selected</c:if>>PENDING
											DOCUMENT</option>
										<option value="7" <c:if test="${status eq 7 }">selected</c:if>>PENDING
											INVESTIGATION</option>
										<option value="15"
											<c:if test="${status eq 15 }">selected</c:if>>FINALIZED</option>
										<option value="17"
											<c:if test="${status eq 17 }">selected</c:if>>REFERED</option>
										<option value="18"
											<c:if test="${status eq 18 }">selected</c:if>>GREY
											AREA</option>
										<option value="19"
											<c:if test="${status eq 19 }">selected</c:if>>PRE
											OPEN</option>
										<option value="20"
											<c:if test="${status eq 20 }">selected</c:if>>PRE
											AUTH</option>
										<option value="21"
											<c:if test="${status eq 21 }">selected</c:if>>PRE
											REJECT</option>
										<option value="22"
											<c:if test="${status eq 22 }">selected</c:if>>PRE
											PENDING</option>
										<%
											}
										%>
								...
								<td class="dataLabel"><input title="Search [Alt+Shift+Q]"
									accesskey="Q" onClick="javascript:cari();"
									class="ui-button ui-widget ui-state-default ui-corner-all ui-state-focus"
									name="button" value="Search" type="submit">
								</td>
							</tr>
							<tr>
...
<!-- 									 	<option value="-1">-- All Case Type --</option> -->
					 ...
					 --%>
								<td class="dataLabel" nowrap="nowrap">Case Type:
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<select name="caseCategoryId" class="inputbox"
									id="caseCategoryId" multiple="multiple">

										<!-- 									 	<option value="-1">-- All Case Type --</option> -->

										<%
											String[] caseCategory = (String[]) request
													.getAttribute("caseCategoryIdMulti");
										...
										%>
										<c:set var="caseCategoryId2" value="<%= panjang%>">
										</c:set>
										<option value="1"
											<c:if test="${caseCategoryId2[0] eq \"1\"}">selected="true"</c:if>>
											INPATIENT</option>
										<option value="2"
											<c:if test="${caseCategoryId2[1] eq \"2\"}">selected="true"</c:if>>
											OUTPATIENT</option>
										<option value="3"
											<c:if test="${caseCategoryId2[2] eq \"3\"}">selected="true"</c:if>>
											MATERNITY</option>
										<option value="4"
											<c:if test="${caseCategoryId2[3] eq \"4\"}">selected="true"</c:if>>
											DENTAL</option>
										<option value="5"
											<c:if test="${caseCategoryId2[4] eq \"5\"}">selected="true"</c:if>>
											OPTICAL</option>
										<option value="7"
											<c:if test="${caseCategoryId2[6] eq \"7\"}">selected="true"</c:if>>
											PRE INPATIENT SERVICE</option>
										<option value="6"
											<c:if test="${caseCategoryId2[5] eq \"6\"}">selected="true"</c:if>>
											DR SPECIALIST</option>
										<option value="8"
											<c:if test="${caseCategoryId2[7] eq \"8\"}">selected="true"</c:if>>
											POST INPATIENT SERVICE</option>
										<option value="10"
											<c:if test="${caseCategoryId2[9] eq \"10\"}">selected="true"</c:if>>
											PRE AND POST INPATIENT SERVICE</option>
										<option value="9"
											<c:if test="${caseCategoryId2[8] eq \"9\"}">selected="true"</c:if>>
											MCU</option>
										<option value="11"
											<c:if test="${caseCategoryId2[10] eq \"11\"}">selected="true"</c:if>>
											SURGERY SERVICE</option>
										<option value="12"
											<c:if test="${caseCategoryId2[11] eq \"12\"}">selected="true"</c:if>>
											LAYANAN KHUSUS</option>
										<option value="13"
											<c:if test="${caseCategoryId2[12] eq \"13\"}">selected="true"</c:if>>
											ONE DAY CARE</option>
										<option value="14"
											<c:if test="${caseCategoryId2[13] eq \"14\"}">selected="true"</c:if>>
											DR UMUM PPK1</option>
										<option value="15"
											<c:if test="${caseCategoryId2[14] eq \"15\"}">selected="true"</c:if>>
											DR GIGI PPK1</option>
										...
										<option value="1"
											<c:if test="${caseCategoryId eq \"1\"}">selected="true"</c:if>>
											INPATIENT</option>
										<option value="2"
											<c:if test="${caseCategoryId eq \"2\"}">selected="true"</c:if>>
											OUTPATIENT</option>
										<option value="3"
											<c:if test="${caseCategoryId eq \"3\"}">selected="true"</c:if>>
											MATERNITY</option>
										<option value="4"
											<c:if test="${caseCategoryId eq \"4\"}">selected="true"</c:if>>
											DENTAL</option>
										<option value="5"
											<c:if test="${caseCategoryId eq \"5\"}">selected="true"</c:if>>
											OPTICAL</option>
										<option value="7"
											<c:if test="${caseCategoryId eq \"7\"}">selected="true"</c:if>>
											PRE INPATIENT SERVICE</option>
										<option value="6"
											<c:if test="${caseCategoryId eq \"6\"}">selected="true"</c:if>>
											DR SPECIALIST</option>
										<option value="8"
											<c:if test="${caseCategoryId eq \"8\"}">selected="true"</c:if>>
											POST INPATIENT SERVICE</option>
										<option value="10"
											<c:if test="${caseCategoryId eq \"10\"}">selected="true"</c:if>>
											PRE AND POST INPATIENT SERVICE</option>
										<option value="9"
											<c:if test="${caseCategoryId eq \"9\"}">selected="true"</c:if>>
											MCU</option>
										<option value="11"
											<c:if test="${caseCategoryId eq \"11\"}">selected="true"</c:if>>
											SURGERY SERVICE</option>
										<option value="12"
											<c:if test="${caseCategoryId eq \"12\"}">selected="true"</c:if>>
											LAYANAN KHUSUS</option>
										<option value="13"
											<c:if test="${caseCategoryId eq \"13\"}">selected="true"</c:if>>
											ONE DAY CARE</option>
										<option value="14"
											<c:if test="${caseCategoryId eq \"14\"}">selected="true"</c:if>>
											DR UMUM PPK1</option>
										<option value="15"
											<c:if test="${caseCategoryId eq \"15\"}">selected="true"</c:if>>
											DR GIGI PPK1</option>
										<%
											}
										%>
								...
								<td class="dataLabel" nowrap="nowrap">Refresh Setting: <select
									name="refreshTimerSet" class="inputbox">
										<option value="-1">-- No Refresh --</option>
										<!-- 					<option value="1" <c:if test="${refreshTimerSet eq \"1\"}">selected="true"</c:if>> -->
										<!-- 						1 Minute   -->
										<!-- 					</option> -->
										<option value="2"
											<c:if test="${refreshTimerSet eq \"2\"}">selected="true"</c:if>>
											2 Minutes</option>
										<option value="3"
											<c:if test="${refreshTimerSet eq \"3\"}">selected="true"</c:if>>
											3 Minutes</option>
										<option value="4"
											<c:if test="${refreshTimerSet eq \"4\"}">selected="true"</c:if>>
											4 Minutes</option>
										<option value="5"
											<c:if test="${refreshTimerSet eq \"5\"}">selected="true"</c:if>>
											5 Minutes</option>
								</select>
								</td>
								<td class="dataLabel" nowrap="nowrap">View List: <select
									name="viewCountSet" class="inputbox">
										<option value="15"
											<c:if test="${viewCountSet eq \"15\"}">selected="true"</c:if>>
											-- Default (15 Rows) --</option>
										<option value="20"
											<c:if test="${viewCountSet eq \"20\"}">selected="true"</c:if>>
											20 Rows</option>
										<option value="40"
											<c:if test="${viewCountSet eq \"40\"}">selected="true"</c:if>>
											40 Rows</option>
										<option value="60"
											<c:if test="${viewCountSet eq \"60\"}">selected="true"</c:if>>
											60 Rows</option>
								</select>
								</td>
							</tr>
	...
	<table>
		<tbody>
			<tr>
				<td width="50px" title="Case Open [IP & MA] &#13Pasien Baru"><legend
						class="caseopen"></legend>
				</td>
				<td>Case Open [IP & MA] - Pasien Baru &nbsp;</td>
				<td width="50px" title="Case [IP & MA] &#13Perawatan 2-5 Hari">
					<legend class="level1"></legend>
				</td>
				<td>Case [IP & MA] - Perawatan 2-5 Hari&nbsp;</td>
				<td width="50px" title="Case [IP & MA] &#13Perawatan 6-8 Hari">
					<legend class="level2"></legend>
				</td>
				<td>Case [IP & MA] - Perawatan 6-8 Hari &nbsp;</td>
				<td width="50px" title="Case [IP & MA] &#13Perawatan > 8 Hari">
					<legend class="level3"></legend>
				</td>
				<td>Case [IP & MA] - Perawatan > 8 Hari &nbsp;</td>
				<td width="50px" title="Case Bukan [IP & MA]"><legend
						class="other"></legend>
				</td>
				<td>Case Bukan [IP & MA]</td>
			</tr>
		</tbody>
	</table>
 ...
 %> <span class="pageNumbers">(<c:out value="${minIndex}" /> - <c:out
											value="${maxIndex}" /> of <c:out value="${count}" />)</span>&nbsp;&nbsp;

									<%
										if (totalIndex > index) {
									%> <a href="javascript:goright()"
									class="listViewPaginationLinkS1">Next&nbsp; <img
										src="images/next.gif" alt="Next" align="absmiddle" border="0"
										height="10" width="4"> </a>&nbsp;&nbsp; <a
									href="javascript:gorightbgt()" class="listViewPaginationLinkS1">End&nbsp;
										<img src="images/end.gif" alt="End" align="absmiddle"
 ...
 %>
			...
			<tr height="20">
				...
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="8%">Case
					Number &nbsp; <input type="hidden" name="sortByOrderCaseNo"
					value="<c:out value="${sortCaseNumber?'true':'false'}" />">
					<a href="javascript:carisort('casenumber')"
					class="listViewThLinkS1"> <img
						src="images/arrow_<c:out value="${sortCaseNumber?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="10%">Member
					Name &nbsp; <input type="hidden" name="sortByOrderMember"
					value="<c:out value="${sortMemberName?'true':'false'}" />">
					<a href="javascript:carisort('membername')"
					class="listViewThLinkS1"> <img
						src="images/arrow_<c:out value="${sortMemberName?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="10%">Client
					&nbsp; <input type="hidden" name="sortByOrderClient"
					value="<c:out value="${sortClient?'true':'false'}" />"> <a
					href="javascript:carisort('clientname')" class="listViewThLinkS1">
						<img
						src="images/arrow_<c:out value="${sortClient?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="10%">Group
					&nbsp; <input type="hidden" name="sortByOrderGroup"
					value="<c:out value="${sortGroup?'true':'false'}" />"> <a
					href="javascript:carisort('groupname')" class="listViewThLinkS1">
						<img
						src="images/arrow_<c:out value="${sortGroup?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>

				<td scope="col" class="listViewThS1" nowrap="nowrap" width="5%">Admission
					&nbsp; <input type="hidden" name="sortByOrderAdmission"
					value="<c:out value="${sortAdmissionDate?'true':'false'}" />">
					<a href="javascript:carisort('admissiondate')"
					class="listViewThLinkS1"> <img
						src="images/arrow_<c:out value="${sortAdmissionDate?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="5%">Discharge
					&nbsp; <input type="hidden" name="sortByOrderDischarge"
					value="<c:out value="${sortDischargeDate?'true':'false'}" />">
					<a href="javascript:carisort('dischargedate')"
					class="listViewThLinkS1"> <img
						src="images/arrow_<c:out value="${sortDischargeDate?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="2%">Type
					&nbsp; <input type="hidden" name="sortByOrderType"
					value="<c:out value="${sortType?'true':'false'}" />"> <a
					href="javascript:carisort('type')" class="listViewThLinkS1"> <img
						src="images/arrow_<c:out value="${sortType?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>

				<td scope="col" class="listViewThS1" nowrap="nowrap" width="3%">Diagnosis
					&nbsp; <input type="hidden" name="sortByOrderDiagnosis"
					value="<c:out value="${sortDiagnosis?'true':'false'}" />">
					<a href="javascript:carisort('diagnosis')" class="listViewThLinkS1">
						<img
						src="images/arrow_<c:out value="${sortDiagnosis?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="5%">Charge
					&nbsp; <input type="hidden" name="sortByOrderCharge"
					value="<c:out value="${sortCharge?'true':'false'}" />"> <a
					href="javascript:carisort('charge')" class="listViewThLinkS1">
						<img
						src="images/arrow_<c:out value="${sortCharge?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" nowrap="nowrap" width="5%">Approved
					&nbsp; <input type="hidden" name="sortByOrderApprove"
					value="<c:out value="${sortAprrove?'true':'false'}" />"> <a
					href="javascript:carisort('approve')" class="listViewThLinkS1">
						<img
						src="images/arrow_<c:out value="${sortAprrove?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" width="5%">Excess &nbsp; <input
					type="hidden" name="sortByOrderExcess"
					value="<c:out value="${sortExcess?'true':'false'}" />"> <a
					href="javascript:carisort('excess')" class="listViewThLinkS1">
						<img
						src="images/arrow_<c:out value="${sortExcess?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
				<td scope="col" class="listViewThS1" width="5%">Status &nbsp; <input
					type="hidden" name="sortByOrderStatus"
					value="<c:out value="${sortStatus?'true':'false'}" />"> <a
					href="javascript:carisort('status')" class="listViewThLinkS1">
						<img
						src="images/arrow_<c:out value="${sortStatus?'up':'down'}" />.gif"
						alt="Sort" align="absmiddle" border="0" height="9" width="8">
				</a>
				</td>
			...
			<c:forEach items="${Cases}" var="myCase" varStatus="status">
				...
				<c:choose>
					<c:when
						test="${myCase.caseCategoryId.caseCategoryCode eq 'IP' or myCase.caseCategoryId.caseCategoryCode eq 'MA' }">
						<c:choose>
							<c:when
								test="${(myCase.caseStatusId.caseStatusId ne 4) and (myCase.caseStatusId.caseStatusId ne 15) }">
								...
								<c:choose>
									<c:when
										test="${(interval eq 1) and (myCase.caseStatusId.caseStatusId eq 1)}">
				...
				<!-- style="color:<c:out value="${fontcolorset eq 'default'? '' : fontcolorset}" />"> -->
				<tr height="20" style="border-top: 1px solid #000;"
					class="<c:out value="${casecolorset}" />">
					...
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						...
						class="linkDetail"
						style="color:#444">
							<c:out value="${myCase.caseNumber}" /> </a>
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top">
						<!-- modified by aju on 20150925, as Pak Firman request for provider -->
						<%-- 
			<a class="linkDetail" style="color:<c:out value="${fontcolorset eq 'default'? '' : fontcolorset}" />" href="member?navigation=detail&memberId=<c:out value="${myCase.memberId.memberId}" />"><c:out value="${myCase.memberId.firstName}" /> <c:out value="${myCase.memberId.lastName}" />
			 --%> <a class="linkDetail"
						style="color:#444"
						href="#"><c:out value="${myCase.memberId.firstName}" /> <c:out
								value="${myCase.memberId.lastName}" /> <c:if
								test="${myCase.memberId.isVIP eq 1}">[VIP]</c:if> </a>
						<div
							style="color:#444">
							(
					...
					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top"><c:out
							value="${myCase.memberId.clientName}" />
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top"><c:out
							value="${myCase.memberId.groupName}" />
					</td>

					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out value="${myCase.caseStartTime}" />
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out value="${myCase.caseEndTime}" />
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out
							value="${myCase.caseCategoryId.caseCategoryCode}" />
					</td>

					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top"><c:out
							value="${myCase.diagnosis1Id.diagnosisCode}" />
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top"
						align="right"><fmt:formatNumber>
							<c:out value="${myCase.caseClaimValue}" />
						</fmt:formatNumber>
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top"
						align="right"><fmt:formatNumber>
							<c:out value="${myCase.caseApprovedValue}" />
						</fmt:formatNumber>
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top"
						align="right"><fmt:formatNumber>
					...
					<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top"
						style="font-weight: bold;"><c:if
							test="${myCase.caseStatusId.caseStatusId eq -1}">VOID</c:if> <c:if
							test="${myCase.caseStatusId.caseStatusId eq 1}">OPEN</c:if> <c:if
							test="${myCase.caseStatusId.caseStatusId eq 2}">PENDING DOCUMENT</c:if>
...
<%String nav = "";
			if (request.getAttribute("navigation").equals("gosearch")
					|| request.getAttribute("navigation").equals("golookup")) {
