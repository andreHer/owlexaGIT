...
<form name="form1" action="paymenttransactionlog" method="POST">
...
<input type="hidden" name="index" value="<c:out value="${index}" />">
  ...
  <tbody>
    <tr>
      <td nowrap="nowrap"><h3><img src="images/h3Arrow.gif"  border="0">&nbsp;Search Payment Transaction Log</h3></td>
            ...
            <td class="dataLabel" nowrap="nowrap"><slot>Search Category:</slot>
              ...
              <slot>
 		   			...
 		   			<option value="claimNumber" <c:if test="${searchby eq \"claimNumber\"}">selected="true"</c:if> >Claim Number</option>
			   			<option value="noVoucherNumber" <c:if test="${searchby eq \"noVoucherNumber\"}">selected="true"</c:if> >No Voucher Number</option>
			   		   			<option value="currency" <c:if test="${searchby eq \"currency\"}">selected="true"</c:if> >Currency</option>
...
<!-- ini default generated table from database -->
					...
					<td scope="col" class="listViewThS1" nowrap="nowrap" width="10%">No Voucher Number</td>
			
		
			   		   			
					<td scope="col" class="listViewThS1" nowrap="nowrap" width="10%">Currency</td>
			
		
			   		   			
					<td scope="col" class="listViewThS1" nowrap="nowrap" width="10%">Bank Name</td>
			
		
			   			
					<td scope="col" class="listViewThS1" nowrap="nowrap" width="10%">Account Name</td>
			
		
			   		   		   	   <td scope="col" class="listViewThS1" nowrap="nowrap" width="10%"><slot>&nbsp;</slot>
	   </td>
	</tr>


	<c:forEach items="${PaymentTransactionLogs}" var="paymentTransactionLog" varStatus="status" >
	<%	if (i % 2 == 0) {
			rowclass = "col1";
		} else if (i % 2 != 0) {
			rowclass= "col2";
	...
	%>
	 <tr height="20">
      <td class="oddListRowS1" align="center" bgcolor="#e7f0fe" nowrap="nowrap" valign="top"><%=(i+((indexint-1)*countSet))%></td>
			...
			<slot>
				<c:out value="${paymentTransactionLog.claimNumber}" />
			</slot>
		</td>
					   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			<slot>
				<c:out value="${paymentTransactionLog.noVoucherNumber}" />
			</slot>
		</td>
					   		   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			<slot>
				<c:out value="${paymentTransactionLog.currency}" />
			</slot>
		</td>
					   		   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			<slot>
				<c:out value="${paymentTransactionLog.bankName}" />
			</slot>
		</td>
					   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			<slot>
				<c:out value="${paymentTransactionLog.accountName}" />
			</slot>
		</td>
					   		   		         <td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" align="center" valign="top">
		<a href="javascript:detil('<c:out value="${paymentTransactionLog.idProses}" />')">
				<img src="images/view.gif" class="action_icon" alt="View" title="View"></a>

			<!-- ini default edit table for each data -->
			<a href="javascript:ubah('<c:out value="${paymentTransactionLog.idProses}" />')">
				<img src="images/edit.gif" class="action_icon" alt="Edit" title="Edit"></a>

			<!-- ini default delete table for each data -->
			<a href="javascript:hapus('<c:out value="${paymentTransactionLog.idProses}" />')">
...
function hapus (idx){
	...
	if (delConfirm){
		...
		document.form1.idProses.value = idx;
		document.form1.action = "paymenttransactionlog";
