...
<form name="form1" method="GET" action="conversationcategory" >

 search By :
 <select name="searchby">

 		   			<option name="conversationCategoryName" <c:if test="${searchby eq \"conversationCategoryName\"}">selected="true"</c:if> >conversationCategoryName</option>
			   			...
			   			<option name="createdBy" <c:if test="${searchby eq \"createdBy\"}">selected="true"</c:if> >createdBy</option>
			   		   			<option name="updatedBy" <c:if test="${searchby eq \"updatedBy\"}">selected="true"</c:if> >updatedBy</option>
...
<input type="hidden" name="arah" value="">
	<input type="hidden" name="conversationCategoryId" value="<c:out value="${conversationCategory.conversationCategoryId}" />">
...
<table width="95%" height="25" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<!-- ini default generated table from database -->
		   			<td width="30" align="center">Conversation Category Name</td>
			   			<td width="30" align="center">Description</td>
			   			<td width="30" align="center">Created By</td>
			   		   			<td width="30" align="center">Updated By</td>
			   		   		   			<td width="30" align="center">Deleted By</td>
			   		   		<td width="30" align="center">Pilih</td>
	</tr>

	<c:forEach items="${ConversationCategorys}" var="conversationCategory" varStatus="status" >
	<tr>

		<td width="30" align="center"><c:out value="${status.count}" /></td>
		<!-- ini default generated table from database -->
		   			<td width="30" align="center"><c:out value="${conversationCategory.conversationCategoryName}" /></td>
			   			<td width="30" align="center"><c:out value="${conversationCategory.description}" /></td>
			   			<td width="30" align="center"><c:out value="${conversationCategory.createdBy}" /></td>
			   		   			<td width="30" align="center"><c:out value="${conversationCategory.updatedBy}" /></td>
			   		   		   			<td width="30" align="center"><c:out value="${conversationCategory.deletedBy}" /></td>
			   		   
		<td width="30" align="center">
			<!-- ini default edit table for each data -->
			<input type="button" name="pilihsaya" onClick="javascript:pilih('<c:out value="${conversationCategory.conversationCategoryId}" />')" value="Pilih">
		</td>
	</tr>
	</c:forEach>
	</table>
...
function pilih (idx){
	window.opener.setConversationCategory (idx);
	window.close();
}
function goleft(){
...
function cari (){
	document.form1.navigation.value = "golookup";
	document.form1.action = "conversationcategory";
	document.form1.method = "GET";
	document.form1.submit();
}
</script>
