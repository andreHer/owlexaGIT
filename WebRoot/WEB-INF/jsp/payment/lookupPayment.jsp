...
<form name="form1" method="GET" action="payment" >

 search By :
 <select name="searchby">

 		   		   		   		   		   			<option name="remarks" <c:if test="${searchby eq \"remarks\"}">selected="true"</c:if> >remarks</option>
			   			<option name="bankName" <c:if test="${searchby eq \"bankName\"}">selected="true"</c:if> >bankName</option>
			   			<option name="accountNumber" <c:if test="${searchby eq \"accountNumber\"}">selected="true"</c:if> >accountNumber</option>
			   			<option name="giroNumber" <c:if test="${searchby eq \"giroNumber\"}">selected="true"</c:if> >giroNumber</option>
			   			<option name="payeeName" <c:if test="${searchby eq \"payeeName\"}">selected="true"</c:if> >payeeName</option>
...
<table width="95%" height="25" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<!-- ini default generated table from database -->
			   			...
			   			<td width="30" align="center">Giro Number</td>
			   			<td width="30" align="center">Payee Name</td>
	...
	<tr>
		...
		<!-- ini default generated table from database -->
		   		   		   		   		   			<td width="30" align="center"><c:out value="${payment.remarks}" /></td>
			   			<td width="30" align="center"><c:out value="${payment.bankName}" /></td>
			   			<td width="30" align="center"><c:out value="${payment.accountNumber}" /></td>
			   			<td width="30" align="center"><c:out value="${payment.giroNumber}" /></td>
			   			<td width="30" align="center"><c:out value="${payment.payeeName}" /></td>
			   		   		   		   			<td width="30" align="center"><c:out value="${payment.createdBy}" /></td>
			   		   			<td width="30" align="center"><c:out value="${payment.deletedBy}" /></td>
			   		   			<td width="30" align="center"><c:out value="${payment.modifiedBy}" /></td>
			   		   
		<td width="30" align="center">
			<!-- ini default edit table for each data -->
			<input type="button" name="pilihsaya" onClick="javascript:pilih('<c:out value="${payment.paymentId}" />')" value="Pilih">
		</td>
	</tr>
	</c:forEach>
	</table>
