    ...
    <tr>
	  <c:choose>
		<c:when test="${navigation eq 'update'}">
     		<td nowrap="nowrap"><h3><img src="images/h3Arrow.gif"  border="0">&nbsp;Update Payment</h3></td>
     	</c:when>
		<c:when test="${navigation ne 'update'}">
     		<td nowrap="nowrap"><h3><img src="images/h3Arrow.gif"  border="0">&nbsp;Register Payment</h3></td>
     	</c:when>
	  </c:choose>
...
<table class="tabForm" border="0" cellpadding="0" cellspacing="0" width="100%">
	<tbody>		
			...
			<spring:bind path="paymentForm.paymentRecipient">
				<input class="inputtext" type="text" size="35" name="<c:out value="${status.expression}" />" value="<c:out value="${status.value}" />" <c:if test="${navigation eq 'confirm' }">readonly</c:if>/>
		...
		<tr>
		 <td class="dataLabel"> Submission Date : </td>				
						...
						<td class="dataField">
			<spring:bind path="paymentForm.paymentTime">
					<input type="text" name="<c:out value="${status.expression}" />" id="<c:out value="${status.expression}"/>" value="<c:out value="${status.value}" />" <c:if test="${navigation eq 'confirm' }">readonly</c:if> maxlength="10">
					<c:if test="${navigation ne 'confirm' }"><img src="images/jscalendar.gif" alt="Enter Date" id="<c:out value="${status.expression}"/>_trigger" align="absmiddle" height="20" width="20"></c:if>
			...
			<td class="dataField">
			<spring:bind path="paymentForm.claimPaymentValue">
					<input type="text" size="35" name="<c:out value="${status.expression}" />" value="<c:out value="${status.value}" />" maxlength="22" <c:if test="${navigation eq 'confirm' }">readonly</c:if>>
				<div id="fredcaption">
					<c:out value="${status.errorMessage}" />
				</div>
			</spring:bind>
	    ...
	    <td class="dataLabel"> Administration Cost : </td>				
			...
			<td class="dataField">
			<spring:bind path="paymentForm.administrationCost">
			...
			<spring:bind path="paymentForm.bankName">
				<input class="inputtext" type="text" size="35" id="<c:out value="${status.expression}" />"  name="<c:out value="${status.expression}" />" value="<c:out value="${status.value}" />" <c:if test="${navigation eq 'confirm' }">readonly</c:if>/>
	...
	<c:if test="${navigation eq 'confirm'}">
	    ...
	    <td class="dataLabel"> Payment Date : </td>				
						...
						<td class="dataField">
			<spring:bind path="paymentForm.paymentConfirmDate">
	...
	<tr>
			...
			<td class="dataField">
			<spring:bind path="paymentForm.confirmedPaymentValue">
				<input class="inputtext" type="text" id="confirmedValue" size="35" name="<c:out value="${status.expression}" />" value="<c:out value="${status.value}" />" />
			
				<div id="fredcaption">
					<c:out value="${status.errorMessage}" />
				</div>
			</spring:bind>
			<input type="hidden" name="valueChecked" id="valueChecked" value="" />
		</td>
	    <td class="dataLabel">  </td>				
			...
			<tr>
				<td class="dataLabel"> Remarks : <div id="paymentRemarks_feedback"></td>				
			

		    <td class="dataField" colspan="3">
			<spring:bind path="paymentForm.remarks">
			<textarea id="paymentRemarks" maxlength="4000" rows="8" cols="45" name="<c:out value="${status.expression}"/>"><c:out value="${status.value}" /></textarea>
				<div id="fredcaption">
					<c:out value="${status.errorMessage}" />
				</div>
			</spring:bind>
  ...
  <tbody>
    <tr>
      <td style="padding-top: 2px;">
        ...
        <c:if test="${navigation eq 'confirm' }">
        <input title="Confirm Payment [Alt+Shift+S]" accesskey="S" class="ui-button ui-widget ui-state-default ui-corner-all ui-state-focus" onClick="javascript:confirmPayment()" name="ConfirmPay" value=" Confirm Payment " type="submit">
        </c:if>
...
<table class="listView" cellspacing="0" cellpadding="0" width="100%">
	...
	<tr height="20">
		...
		<td scope="col" class="listViewThS1" nowrap="nowrap" width="5%">Approve &nbsp;</td>		
	...
	<c:forEach items="${claimList}" var="claim" varStatus="status" >
	 ...
	 <tr height="20">
		...
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">			
			<a href="claim?navigation=detail&index=<c:out value="${index}" />&claimId=<c:out value="${claim.claimId}" />" class="linkDetail"><c:out value="${claim.claimNumber}" /></a>			
		</td>
		   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
		...
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
				...
				<c:out value="${claim.claimChargeValue}" />
			...
			<input type="checkbox" onclick="javascript:updateCheck();" name="claimList" value="<c:out value="${claim.claimId}" />">
	  </td>		   		   		
	...
	<tr height="20">
      	<td class="listViewThS1" align="center" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
      	</td>			
		...
		<td class="listViewThS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
		   	...
		   	<td class="listViewThS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			   		         ...
			   		         <td class="listViewThS1" bgcolor="#e7f0fe" nowrap="nowrap" style="text-align: center;" align="center" valign="top">
...
$(document).ready(function(){
    ...
    $('#paymentRemarks_feedback').html('('+$('#paymentRemarks').val().length +'/' + $('#paymentRemarks').attr('maxlength') + ')');
	...
	function updateCheck(){
		...
		AJAXClaimService.calculatePayment (toCalc, {
