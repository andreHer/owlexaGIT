...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tbody>
    <tr>
      <td nowrap="nowrap"><h3><img src="images/h3Arrow.gif" alt="Call Search" border="0">&nbsp;Search Discount Usage Type</h3></td>
...
<div class="table_container">
	<!-- Table Toolbar Start -->
		...
		<div id="search_float"<%=nampak%>>
			<h4>
				<c:out value="${SEARCH}" /> Data discountUsageType
			</h4>
			<form name=form1 onSubmit="cari()">
				...
				<label for="status">
				 ...
				 <select name="searchby" class="inputbox">
 		   			...
 		   			<option name="discountUsageTypeName" <c:if test="${searchby eq \"discountUsageTypeName\"}">selected="true"</c:if> >Discount Usage Type Name</option>
...
<BR/>
				...
				<input type="hidden" name="arah" value="">
								<input type="hidden" name="discountUsageTypeId" value="<c:out value="${discountUsageType.discountUsageTypeId}" />">
							</form>
		</div>
	</div>
	<!-- Table Toolbar Stop -->
...
<LABEL><c:out value="${LIST_OF}" />  Discount Usage Type</LABEL>
...
<!-- ini default generated table from database -->
		   			<th>Discount Usage Type Name</th>
			   			<th>Description</th>
			   		<th>Action</th>
...
%>
	<c:forEach items="${DiscountUsageTypes}" var="discountUsageType" varStatus="status" >
	<%	if (i % 2 == 0) {
			rowclass = "col1";
		} else if (i % 2 != 0) {
			rowclass= "col2";
	...
	<tr class="<%=rowclass%>" onmouseover="this.className='highlight'" onmouseout="this.className='<%=rowclass%>'" height="20">
		...
		<!-- ini default generated table from database -->
				...
				<c:out value="${discountUsageType.discountUsageTypeName}" />
			
		</td>
			   			<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${discountUsageType.description}" />
			
		</td>
			   
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" align="center">
			<!-- ini default edit table for each data -->
			<a href="javascript:detil('<c:out value="${discountUsageType.discountUsageTypeId}" />')">
				<img src="images/icon_view.gif" class="action_icon" alt="View" title="View"></a>

			<!-- ini default edit table for each data -->
			<a href="javascript:ubah('<c:out value="${discountUsageType.discountUsageTypeId}" />')">
				<img src="images/icon_edit.gif" class="action_icon" alt="Edit" title="Edit"></a>

			<!-- ini default delete table for each data -->
			<a href="javascript:hapus('<c:out value="${discountUsageType.discountUsageTypeId}" />')">
...
function tambahi (){
	...
	document.form1.action = "discountusagetype-form";
...
function hapus (idx){
	...
	if (delConfirm){
		...
		document.form1.discountUsageTypeId.value = idx;
		document.form1.action = "discountusagetype";
