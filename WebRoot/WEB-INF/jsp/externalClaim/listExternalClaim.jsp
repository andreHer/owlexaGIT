...
<br />
<form name="form1" action="externalclaim" method="POST">
<input type="hidden" name="navigation" value="gosearch">
<input type="hidden" name="arah" value="">
<input type="hidden" name="index" value="<c:out value="${index}" />">
				<input type="hidden" name="id" value="<c:out value="${externalClaim.id}" />">
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tbody>
    <tr>
      <td nowrap="nowrap"><h3><img src="images/h3Arrow.gif"  border="0">&nbsp;Search External Claim</h3></td>
            ...
            <td class="dataLabel" nowrap="nowrap">Search Category:
              &nbsp;&nbsp;
 		   			...
 		   			<option value="claimNumber" <c:if test="${searchby eq 'claimNumber'}">selected="true"</c:if> >Claim Number</option>
			   			<option value="claimCategory" <c:if test="${searchby eq 'claimCategory'}">selected="true"</c:if> >Claim Category</option>
			   			<option value="claimType" <c:if test="${searchby eq 'claimType'}">selected="true"</c:if> >Claim Type</option>
			   			<option value="batchNumber" <c:if test="${searchby eq 'batchNumber'}">selected="true"</c:if> >Batch Number</option>
			   		   			...
			   		   			<option value="claimStatus" <c:if test="${searchby eq 'claimStatus'}">selected="true"</c:if> >Claim Status</option>
			   			...
			   			<option value="cdvNumber" <c:if test="${searchby eq 'cdvNumber'}">selected="true"</c:if> >Cdv Number</option>
			   			...
			   			<option value="providerName" <c:if test="${searchby eq 'providerName'}">selected="true"</c:if> >Provider Name</option>
			   		   		   		   			<option value="location" <c:if test="${searchby eq 'location'}">selected="true"</c:if> >Location</option>
			   		   		   		   			<option value="payTo" <c:if test="${searchby eq 'payTo'}">selected="true"</c:if> >Pay To</option>
			   			<option value="accountNo" <c:if test="${searchby eq 'accountNo'}">selected="true"</c:if> >Account No</option>
...
<table class="listView" cellspacing="0" cellpadding="0" width="100%">
	...
	<tr height="20">
		...
		<td scope="col" class="listViewThS1" nowrap="nowrap" width="5%">Claim Date</td>	   			
		...
		<td scope="col" class="listViewThS1" nowrap="nowrap" width="3%">ICD</td>			   			
	...
	<c:forEach items="${ExternalClaims}" var="externalClaim" varStatus="status" >
	<%	if (i % 2 == 0) {
			rowclass = "col1";
		} else if (i % 2 != 0) {
			rowclass= "col2";
	...
	%>
	 <tr onMouseOver="setPointer(this, '910cbdf4-ffae-5c1a-db84-4434fa305b0a', 'over', '#e7f0fe', '#FFFFFF', '');" onMouseOut="setPointer(this, '910cbdf4-ffae-5c1a-db84-4434fa305b0a', 'out', '#e7f0fe', '#FFFFFF', '');" onMouseDown="setPointer(this, '910cbdf4-ffae-5c1a-db84-4434fa305b0a', 'click', '#e7f0fe', '#FFFFFF', '');" height="20">
      <td class="oddListRowS1" align="center" bgcolor="#e7f0fe" nowrap="nowrap" valign="top"><%=(i+((indexint-1)*countSet))%></td>
				...
				<c:out value="${externalClaim.claimNumber}" />
			
		</td>
					   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${externalClaim.admissionDate}" />
				...
				<c:out value="${externalClaim.memberNumber}" />
			
		</td>
					   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${externalClaim.memberName}" />
			
		</td>
			
					   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${externalClaim.diagnosis1Code}" />
			
		</td>
					   				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${externalClaim.providerName}" />
			
		</td>
                <td class="oddListRowS1" bgcolor="#e7f0fe"  nowrap="nowrap" valign="top" style="text-align: right;">
			
                            <font style="text-align: right;"> <fmt:formatNumber> <c:out value="${externalClaim.claimChargeValue}" /></fmt:formatNumber></font>
			
		</td>
		   <td class="oddListRowS1" bgcolor="#e7f0fe"  nowrap="nowrap" valign="top" style="text-align: right;">
			
                            <font style="text-align: right;"> <fmt:formatNumber> <c:out value="${externalClaim.claimApprovedValue}" /></fmt:formatNumber></font>
...
function tambahi (){
	...
	document.form1.action = "externalclaim-form";
...
function hapus (idx){
	...
	if (delConfirm){
		...
		document.form1.action = "externalclaim";
