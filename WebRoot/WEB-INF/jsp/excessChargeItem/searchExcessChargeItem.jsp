...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tbody>
    <tr>
      <td nowrap="nowrap"><h3><img src="images/h3Arrow.gif" alt="Call Search" border="0">&nbsp;Search Excess Charge Item</h3></td>
...
<div class="table_container">
	<!-- Table Toolbar Start -->
		...
		<div id="search_float"<%=nampak%>>
			<h4>
				<c:out value="${SEARCH}" /> Data excessChargeItem
			</h4>
...
<LABEL><c:out value="${LIST_OF}" />  Excess Charge Item</LABEL>
...
%>
	<c:forEach items="${ExcessChargeItems}" var="excessChargeItem" varStatus="status" >
	<%	if (i % 2 == 0) {
			rowclass = "col1";
		} else if (i % 2 != 0) {
			rowclass= "col2";
	...
	<tr class="<%=rowclass%>" onmouseover="this.className='highlight'" onmouseout="this.className='<%=rowclass%>'" height="20">
		...
		<!-- ini default generated table from database -->
				...
				<c:out value="${excessChargeItem.createdBy}" />
			
		</td>
			   		   			<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${excessChargeItem.modifiedBy}" />
			
		</td>
			   		   			<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${excessChargeItem.deletedBy}" />
			
		</td>
			   		   
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" align="center">
			<!-- ini default edit table for each data -->
			<a href="javascript:detil('<c:out value="${excessChargeItem.excessChargeItemId}" />')">
				<img src="images/icon_view.gif" class="action_icon" alt="View" title="View"></a>

			<!-- ini default edit table for each data -->
			<a href="javascript:ubah('<c:out value="${excessChargeItem.excessChargeItemId}" />')">
				<img src="images/icon_edit.gif" class="action_icon" alt="Edit" title="Edit"></a>

			<!-- ini default delete table for each data -->
			<a href="javascript:hapus('<c:out value="${excessChargeItem.excessChargeItemId}" />')">
...
function hapus (idx){
	...
	if (delConfirm){
		...
		document.form1.excessChargeItemId.value = idx;
		document.form1.action = "excesschargeitem";
