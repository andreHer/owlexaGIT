				...
				if (input.trim().length() != 10  ){
					...
					if (matcher.matches()){
						if (policy.getEffectiveDate() != null && policy.getExpireDate() != null){
							Date expireDate = Date.valueOf(input);
							
							if ((policy.getEffectiveDate().before(expireDate))
									&& (policy.getExpireDate().after(expireDate) || policy.getExpireDate().equals(expireDate))){
