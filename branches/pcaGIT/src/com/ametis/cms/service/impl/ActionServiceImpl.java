...
/**
 * Action is a servlet controller for action Table.
...

{
	...
	public void setActionDao (ActionDao object){
		this.actionDao = object;
	}
	...
	*/
	public Action create (Action object,ActionUser actionUser) throws Exception {
...
//			object.setCreatedDate (new java.sql.Date (System.currentTimeMillis()));
			...
			if (!isExist(object.getActionCode())){
				result = actionDao.create (object);
			}
			
			return result;
		}
	/*
	* Method update (Action object) berfungsi untuk melakukan perubahan terhadap
	* sebuah object yang terdapat didalam database
	* @param object adalah sebuah object yang ingin diubah
	* @return object hasil update, exception jika gagal
	*/
	public Action update (Action object,ActionUser actionUser) throws Exception{
...
//		object.setUpdatedDate (new java.sql.Date (System.currentTimeMillis()));
	...
	* place
	*/
	public Action trash (java.io.Serializable pkey) throws Exception {
		Action object = actionDao.get (pkey);
	...
	*/
	...
	public Action delete (java.io.Serializable pkey,ActionUser actionUser) throws Exception{
		Action object = actionDao.get (pkey);
...
//		object.setDeletedDate (new java.sql.Date (System.currentTimeMillis()));
	...
	*/
	...
	public Action delete (Action object,ActionUser actionUser) throws Exception{
...
// -- get section - carefull !
	...
	*/
	public Action get (java.io.Serializable pkey) throws Exception{
		Action object = null;
		object = actionDao.get(pkey);
		return object;
	}
	/*
	* Method get (Object pkey) berfungsi untuk melakukan retrieval terhadap
	...
	*/
	...
	public Action get (java.io.Serializable pkey, String[] required) throws Exception{
	    Action object = null;
	    object = actionDao.get(pkey);
...
// -- 1
	public Collection search (String[] likeColumns, Object[] likeParams,
		...
		Criteria c = actionDao.getCriteria();
...
// unique Result
	...
	private boolean isExist (String actionCode) throws Exception{
		boolean result = false;
		
		if (actionCode != null){
			Collection<Action> actions = search(null, null, "actionCode",actionCode,0,10);
	...
	public Action searchUnique (String eqColumns, Object eqParams, String[] required)
		throws Exception{
		Criteria c = actionDao.getCriteria();
		DaoSupportUtil.setEqParam(eqColumns,eqParams,c);
		Action obj = (Action) c.uniqueResult();
		DaoSupportUtil.lazyInit(required,obj);
		return obj;
	}
	public Action searchUnique (String eqColumns, Object eqParams)
		throws Exception{
		Criteria c = actionDao.getCriteria();
		DaoSupportUtil.setEqParam(eqColumns,eqParams,c);
		Action obj = (Action) c.uniqueResult();
...
*/
	...
	public DetachedCriteria getDetachedCriteria() throws Exception {
		DetachedCriteria dc = actionDao.getDetachedCriteria();
		return dc;
	}

	public Collection<Action> getUnassignedAction (Integer roleId,String searchKeyword,String category, 
			int index, int offset) throws Exception{

		/*
		DetachedCriteria dc = roleActionDao.getDetachedCriteria();
		...
		DaoSupportUtil.setNotIn("roleId", dc, c);
		...
		*/
		...
		if (category != null && category.equalsIgnoreCase("actionCode")){
			searchCriteria = " action.actionCode LIKE :code and ";
		}
		else if (category != null && category.equalsIgnoreCase("actionName")){
			searchCriteria = " action.actionName LIKE :name and ";
		}
		Session session = actionDao.getActionSession();
		Query query =  session.createQuery("select action from Action action where "+searchCriteria+" action.actionId not in (select ra.actionId FROM RoleAction ra WHERE ra.roleId = :roleId)");
		query.setInteger("roleId", roleId);
		
		if (category != null && category.equalsIgnoreCase("actionCode")){
			query.setString("code", searchKeyword);
		}
		else if (category != null && category.equalsIgnoreCase("actionName")){
			query.setString("name", searchKeyword);
		}
	...
	public int getTotalUnassignedAction (Integer roleId,String searchKeyword,String category	) throws Exception{
