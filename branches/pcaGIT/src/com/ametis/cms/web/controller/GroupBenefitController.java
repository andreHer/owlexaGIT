...
*/
public class GroupBenefitController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEGROUPBENEFIT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEGROUPBENEFIT access");
				return errorResult;
				
			}
			GroupBenefit res = groupBenefitService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.groupbenefit",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.groupbenefit",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchGroupBenefit");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.groupbenefit",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("groupBenefit", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = groupBenefitService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"GroupBenefit.MemberGroupId",
				"GroupBenefit.ItemCategoryId",
		// foreign affairs end
	};
		    collection = groupBenefitService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("GroupBenefits",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailGroupBenefit" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupGroupBenefit" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchGroupBenefit" );
	    }
