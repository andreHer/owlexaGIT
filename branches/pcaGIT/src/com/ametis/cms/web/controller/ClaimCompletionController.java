 ...
 */
public class ClaimCompletionController implements Controller
...
// extends-
{
	...
	private ClaimCompletionService claimCompletionService;
...
private ActivityLogService logService;
	...
	public void setClaimCompletionService(
			ClaimCompletionService claimCompletionService) {
		this.claimCompletionService = claimCompletionService;
	}
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECLAIMCOMPLETION");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECLAIMCOMPLETION access");
				return errorResult;
				
			}
			ClaimCompletion res = claimCompletionService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.claimcompletion", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.claimcompletion", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchClaimCompletion");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILCLAIMCOMPLETION");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILCLAIMCOMPLETION access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.claimcompletion", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("claimCompletion", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHCLAIMCOMPLETION");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHCLAIMCOMPLETION access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("completionRemarks")) {
						vLikeP.add("completionRemarks");
			...
			count = claimCompletionService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = claimCompletionService.search(sLikeP, sLikeQ, sEqP,
			...
			if (collection.size() <= 0) {
				...
				collection = claimCompletionService.search(sLikeP, sLikeQ,
			...
			result.addObject("ClaimCompletions", collection);
