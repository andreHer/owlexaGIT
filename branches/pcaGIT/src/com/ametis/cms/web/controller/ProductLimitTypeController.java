...
*/
public class ProductLimitTypeController implements Controller
...

{
	...
	private ProductLimitTypeService  productLimitTypeService;
...
private ActivityLogService logService;
	...
	public void setProductLimitTypeService	(ProductLimitTypeService productLimitTypeService){
	this.productLimitTypeService = productLimitTypeService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEPRODUCTLIMITTYPE");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEPRODUCTLIMITTYPE access");
				return errorResult;
				
			}
			ProductLimitType res = productLimitTypeService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.productlimittype",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.productlimittype",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProductLimitType");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.productlimittype",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("productLimitType", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("productLimit")){
					vLikeP.add("productLimit");
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = productLimitTypeService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = productLimitTypeService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProductLimitTypes",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProductLimitType" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProductLimitType" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProductLimitType" );
	    }
