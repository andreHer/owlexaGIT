...
/**
 * CaseProvider is a servlet controller for case_provider Table. All you have to
 * do is to convert necessary data field to the named parameter
 */
public class CaseProviderController implements Controller
...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer caseProviderId = WebUtil.getParameterInteger(request,
					"caseProviderId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECASEPROVIDER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECASEPROVIDER access");
				return errorResult;
				
			}
			
			CaseProvider res = caseProviderService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.caseprovider", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.caseprovider", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchCaseProvider");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILCASEPROVIDER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILCASEPROVIDER access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.caseprovider", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("caseProvider", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHCASEPROVIDER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHCASEPROVIDER access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("rumahSakit")) {
						vLikeP.add("rumahSakit");
			...
			count = caseProviderService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = caseProviderService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = caseProviderService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("CaseProviders", collection);
