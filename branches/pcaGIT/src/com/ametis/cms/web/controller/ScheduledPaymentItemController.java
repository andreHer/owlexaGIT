...
*/
public class ScheduledPaymentItemController implements Controller
...

{
	...
	private ScheduledPaymentItemService  scheduledPaymentItemService;
...
private ActivityLogService logService;
	...
	public void setScheduledPaymentItemService	(ScheduledPaymentItemService scheduledPaymentItemService){
	this.scheduledPaymentItemService = scheduledPaymentItemService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ScheduledPaymentItem res = scheduledPaymentItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.scheduledpaymentitem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.scheduledpaymentitem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchScheduledPaymentItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.scheduledpaymentitem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("scheduledPaymentItem", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = scheduledPaymentItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = scheduledPaymentItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ScheduledPaymentItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailScheduledPaymentItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupScheduledPaymentItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchScheduledPaymentItem" );
	    }
