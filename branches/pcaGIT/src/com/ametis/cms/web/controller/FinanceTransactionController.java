...
*/
public class FinanceTransactionController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			FinanceTransaction res = financeTransactionService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.financetransaction",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.financetransaction",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchFinanceTransaction");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.financetransaction",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("financeTransaction", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("transactionNumber")){
					vLikeP.add("transactionNumber");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("accountCode")){
					vLikeP.add("accountCode");
					vLikeQ.add(searchtext);
				}
					...
					vLikeQ.add(searchtext);
							    							    							    							    							    								...
							    							    							    							    							    								if (searchby.equalsIgnoreCase("referenceCode")){
					vLikeP.add("referenceCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = financeTransactionService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = financeTransactionService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("FinanceTransactions",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailFinanceTransaction" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupFinanceTransaction" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchFinanceTransaction" );
	    }
