...
*/
public class DiagnosisMedicineCategoryController implements Controller
...

{
	...
	private DiagnosisMedicineCategoryService  diagnosisMedicineCategoryService;
	...
	public void setDiagnosisMedicineCategoryService	(DiagnosisMedicineCategoryService diagnosisMedicineCategoryService){
	this.diagnosisMedicineCategoryService = diagnosisMedicineCategoryService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			DiagnosisMedicineCategory res = diagnosisMedicineCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.diagnosismedicinecategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.diagnosismedicinecategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchDiagnosisMedicineCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.diagnosismedicinecategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("diagnosisMedicineCategory", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = diagnosisMedicineCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = diagnosisMedicineCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("DiagnosisMedicineCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailDiagnosisMedicineCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupDiagnosisMedicineCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchDiagnosisMedicineCategory" );
	    }
