...
*/
public class PolicyPaymentController implements Controller
...
private ActivityLogService logService;
	...
	public void setPolicyPaymentService	(PolicyPaymentService policyPaymentService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			PolicyPayment res = policyPaymentService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.policypayment",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.policypayment",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPolicyPayment");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.policypayment",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("policyPayment", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = policyPaymentService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		...
		// foreign affairs end
		    ...
		    collection = policyPaymentService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("PolicyPayments",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPolicyPayment" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPolicyPayment" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPolicyPayment" );
	    }
