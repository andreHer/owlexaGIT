...
*/
public class TreatmentRiskController implements Controller
...
// extends- 
{
	...
	private TreatmentRiskService  treatmentRiskService;
	...
	public void setTreatmentRiskService	(TreatmentRiskService treatmentRiskService){
	this.treatmentRiskService = treatmentRiskService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			TreatmentRisk res = treatmentRiskService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.treatmentrisk",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.treatmentrisk",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchTreatmentRisk");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.treatmentrisk",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("treatmentRisk", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("treatmentRiskName")){
					vLikeP.add("treatmentRiskName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("treatmentRiskCode")){
					vLikeP.add("treatmentRiskCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = treatmentRiskService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = treatmentRiskService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("TreatmentRisks",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailTreatmentRisk" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupTreatmentRisk" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchTreatmentRisk" );
	    }
