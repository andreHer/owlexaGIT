...
// extends- 
{
	...
	public ModelAndView deletePerformed (HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			ExchangeRate res = exchangeRateService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.exchangerate",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.exchangerate",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchExchangeRate");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.exchangerate",null,"fail",Locale.getDefault()));
			}
			
		    result.addObject ("exchangeRate", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
				...
				if (searchby.equalsIgnoreCase("currencyExchangeName")){
					vLikeP.add("currencyExchangeName");
			...
			count = exchangeRateService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = exchangeRateService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ExchangeRates",collection);
...
*/
	...
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailExchangeRate" );
	    }
	    	...
	    	result = searchPerformed (request, response, "searchExchangeRate" );
	    	breadcrumb = "<a href=\"exchangerate\" class=\"linkbreadcrumb\">Search Exchange Rate</a>";
	    }
	    else if (navigation.equalsIgnoreCase ("lookup") || navigation.equals("golookup")){
	    	result = searchPerformed (request, response, "lookupExchangeRate" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchExchangeRate" );
	    	breadcrumb = "<a href=\"exchangerate\" class=\"linkbreadcrumb\">Search Exchange Rate</a>";
