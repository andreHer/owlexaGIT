...
/**
 * InsurancePackage is a servlet controller for insurance_package Table. All you
 * have to do is to convert necessary data field to the named parameter
 */
public class InsurancePackageController implements Controller
...
private ActivityLogService logService;
	...
	public void setInsurancePackageService(
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer packageId = WebUtil.getParameterInteger(request,
					"packageId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEINSURANCEPACKAGE");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEINSURANCEPACKAGE access");
				return errorResult;
				
			}
			InsurancePackage res = insurancePackageService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.insurancepackage", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.insurancepackage", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response,
					"searchInsurancePackage");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.insurancepackage", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("insurancePackage", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("packageName")) {
						vLikeP.add("packageName");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("packageCode")) {
						vLikeP.add("packageCode");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("packageDescription")) {
						vLikeP.add("packageDescription");
						vLikeQ.add(searchtext);
					}
			...
			// foreign affairs
					"InsurancePackage.ClientId",
					"InsurancePackage.PackageType",
			// foreign affairs end
			};
			collection = insurancePackageService.search(sLikeP, sLikeQ, sEqP,
			...
			if (collection.size() <= 0) {
				...
				collection = insurancePackageService.search(sLikeP, sLikeQ,
			...
			result.addObject("InsurancePackages", collection);
