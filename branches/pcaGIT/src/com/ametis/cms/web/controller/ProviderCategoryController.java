...
*/
public class ProviderCategoryController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEPROVIDERCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEPROVIDERCATEGORY access");
				return errorResult;
				
			}
			ProviderCategory res = providerCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.providercategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.providercategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProviderCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.providercategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("providerCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("providerCategoryCode")){
					vLikeP.add("providerCategoryCode");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("providerCategoryName")){
					vLikeP.add("providerCategoryName");
					vLikeQ.add(searchtext);
				}
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = providerCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = providerCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProviderCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProviderCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProviderCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProviderCategory" );
	    }
