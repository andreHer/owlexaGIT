...
/**
 * CallCategory is a servlet controller for call_category Table. All you have to
 * do is to convert necessary data field to the named parameter
 */
public class CallCategoryController implements Controller
...
private ActivityLogService logService;
	...
	public void setCallCategoryService(CallCategoryService callCategoryService) {
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer callCategoryId = WebUtil.getParameterInteger(request,
					"callCategoryId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECALLCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECALLCATEGORY access");
				return errorResult;
				
			}
			CallCategory res = callCategoryService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.callcategory", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.callcategory", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchCallCategory");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILCALLCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILCALLCATEGORY access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.callcategory", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("callCategory", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHCALLCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHCALLCATEGORY access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("callCategoryName")) {
						vLikeP.add("callCategoryName");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("callCategoryCode")) {
						vLikeP.add("callCategoryCode");
						vLikeQ.add(searchtext);
					}
			...
			count = callCategoryService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = callCategoryService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = callCategoryService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("CallCategorys", collection);
