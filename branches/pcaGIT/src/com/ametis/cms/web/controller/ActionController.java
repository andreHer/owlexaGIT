...
/**
 * Action is a servlet controller for action Table. All you have to do is to
...

{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			 ...
			 * String[] eqP = new String[]{"email"}; String[] eqQ = new
			 * String[]{principal.getUsername()};
			 * 
			 * 
			 * Collection coll = userService.search(null,null,eqP,eqQ,-1,-1);
			 * if(coll==null||coll.size()<=0){ user = new User(); }else{ User
			 * users[] = new User[coll.size()]; coll.toArray(users); user =
			 * users[0]; }
			 * 
			 *  }
			 */
			...
			if (!isUserAuthorized) {
				...
				errorResult.addObject("errorMessage",
						"You Are Not Authorized for DELETEACTION access");
				return errorResult;

			}
			Action res = actionService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.action", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties
						.getMessage("fail.delete.action", null, "fail", Locale
								.getDefault()));

			}

			LogUtil.log(logService, user, "Delete Action", res.toString());
			result = searchPerformed(request, response, "searchAction");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (!isUserAuthorized) {
				...
				errorResult.addObject("errorType", "accessDenied");
				errorResult.addObject("errorMessage",
						"You Are Not Authorized for DETAILACTION access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.action", null, "fail", Locale.getDefault()));
			}
			/*
			...
			LogUtil.log(logService, user, "Detail View Action", object.toString());
			result.addObject("action", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHACTION");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHACTION access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("actionName")) {
						vLikeP.add("actionName");
					...
					if (searchby.equalsIgnoreCase("actionUrl")) {
						vLikeP.add("actionUrl");
						vLikeQ.add(searchtext);
					}
			...
			count = actionService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = actionService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			LogUtil.log(logService, user, "Search Action", "SearchText : " + searchtext + "  SearchBy : " + searchby);
			/*
			 * ini gue generate juga dari nama tablenya convention -->
			 * collection = nama bean + 's' -adhit
			 */

			result.addObject("Actions", collection);
...
LogUtil.log(logService, user, "Search UnAssigned Action", "SearchText : " + searchtext + "  SearchBy : " + searchby);
			result.addObject("Actions", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailAction");
				String actionId = request.getParameter("actionId");
				breadcrumb = "<a href=\"action?navigation=detail&actionId="+actionId+"\" class=\"linkbreadcrumb\">Detail Action</a>";
			} else if (navigation.equalsIgnoreCase("delete")) {
				result = deletePerformed(request, response);
			} else if (navigation.equalsIgnoreCase("search")
					|| navigation.equals("gosearch")) {
				result = searchPerformed(request, response, "searchAction");
				breadcrumb = "<a href=\"action?navigation=search&searchtext=&searchby=&index=\" class=\"linkbreadcrumb\">Search Action</a>";
			} else if (navigation.equalsIgnoreCase("lookup")
					|| navigation.equals("golookup")) {
				result = searchPerformed(request, response, "lookupAction");
			} else {
				result = searchPerformed(request, response, "searchAction");
				breadcrumb = "<a href=\"action?navigation=search&searchtext=&searchby=&index=\" class=\"linkbreadcrumb\">Search Action</a>";
			}
