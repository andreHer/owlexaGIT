...
/**
 * AuthProvider is a servlet controller for auth_provider Table. All you have to
 * do is to convert necessary data field to the named parameter
 */
public class AuthProviderController implements Controller
...
private ActivityLogService logService;
	...
	public void setAuthProviderService(AuthProviderService authProviderService) {
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer authProviderId = WebUtil.getParameterInteger(request,
					"authProviderId");
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEAUTHPROVIDER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEAUTHPROVIDER access");
				return errorResult;
				
			}
			
			AuthProvider res = authProviderService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.authprovider", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.authprovider", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchAuthProvider");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILAUTHPROVIDER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILAUTHPROVIDER access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.authprovider", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("authProvider", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHAUTHPROVIDER");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHAUTHPROVIDER access");
				return errorResult;
			...
			count = authProviderService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = authProviderService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = authProviderService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("AuthProviders", collection);
