...
/**
 * CaseProcedure is a servlet controller for case_procedure Table. All you have
 * to do is to convert necessary data field to the named parameter
 */
public class CaseProcedureController implements Controller
...
// extends-
{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			CaseProcedure res = caseProcedureService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.caseprocedure", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.caseprocedure", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchCaseProcedure");
		} catch (Exception e) {
	...
	public ModelAndView approvePerformed(HttpServletRequest request,
		...
		try {
			...
			CaseProcedure caseProcedure = caseProcedureService.get(pkey);
			...
			if (navigation.equalsIgnoreCase("approve")){
				CaseProcedure proc = caseProcedureService.get(pkey);
				if (proc != null){
					proc.setStatus(1);
					proc.setApprovalRemarks(reason);
					
					caseProcedureService.update(proc, user);
					hasil = true;
				}
			}
			...
			if (hasil) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.approve.caseprocedure", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.caseprocedure", null, "fail", Locale
			...
			result = searchApprovePerformed(request, response, "searchApproveProcedure");
		} catch (Exception e) {
			// nanti kalo sudah OK codenya e.printStackTrace nya di hapus saja
			e.printStackTrace();
			request.setAttribute("alert", alertProperties.getMessage(
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.caseprocedure", null, "fail", Locale
								.getDefault()));
			}
			else {
			...
			result.addObject("caseProcedure", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup") || navigation.equals("list")) {
				...
				if (searchby != null && !searchtext.equals("") && searchtext != null) {
					...
					if (searchby.equalsIgnoreCase("approvalRemarks")) {
						vLikeP.add("approvalRemarks");
						vLikeQ.add(searchtext);
					}
			...
			count = caseProcedureService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = caseProcedureService.search(sLikeP, sLikeQ, sEqP,
...
//					System.out.println("member group object : "+caseObject.getAddress());
			...
			result.addObject("CaseProcedures", collection);
	...
	public ModelAndView jsonTotalPendingPerformed(HttpServletRequest request,
		...
		try {
			...
			int total = caseProcedureService.getTotal(null,null,eqParam,eqValue);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
			...
			else if (navigation.equalsIgnoreCase("approve") || navigation.equalsIgnoreCase("reject")) {
				result = approvePerformed(request, response);
			} 
			 else if (navigation.equalsIgnoreCase("searchapprove")) {
					result = searchApprovePerformed(request, response,"searchApproveProcedure");
				} 
				...
				result = searchPerformed(request, response,
						"searchCaseProcedure");
				breadcrumb = "<a href=\"case?navigation=detail&caseId="+caseId+"\" class=\"linkbreadcrumb\">Detail Case</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Procedure";
			} else {
				result = searchPerformed(request, response,
						"searchCaseProcedure");
				breadcrumb = "<a href=\"case?navigation=detail&caseId="+caseId+"\" class=\"linkbreadcrumb\">Detail Case</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Procedure";
