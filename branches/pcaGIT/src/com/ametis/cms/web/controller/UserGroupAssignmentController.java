...
*/
public class UserGroupAssignmentController implements Controller
...
private ActivityLogService logService;
	...
	public void setUserGroupAssignmentService	(UserGroupAssignmentService userGroupAssignmentService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			UserGroupAssignment res = userGroupAssignmentService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.usergroupassignment",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.usergroupassignment",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchUserGroupAssignment");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.usergroupassignment",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("userGroupAssignment", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = userGroupAssignmentService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = userGroupAssignmentService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("UserGroupAssignments",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailUserGroupAssignment" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupUserGroupAssignment" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchUserGroupAssignment" );
	    }
