...
*/
public class ProductLayerLimitController implements Controller
...
// extends- 
{
	...
	public ModelAndView deletePerformed (HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ModelAndView result = null;
 	    ...
 	    try {
			Integer productLayerLimitId = WebUtil.getParameterInteger (request,"productLayerLimitId");
			
			java.io.Serializable pkey = productLayerLimitId;
						
			ActionUser user = securityService.getActionUser(request);
		
			ProductLayerLimit res = productLayerLimitService.delete (pkey,user);
			String alert = "";

			if (res!=null){
				alert = alertProperties.getMessage ("success.delete.productlayerlimit",null,"success",Locale.getDefault());
			}
			else {
				alert = alertProperties.getMessage ("fail.delete.productlayerlimit",null,"success",Locale.getDefault());
			}
			result = new ModelAndView(new RedirectView("product?navigation=detail&productId="+res.getProductId().getProductId()+"&alert="+alert));
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.productlayerlimit",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("productLayerLimit", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = productLayerLimitService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"ProductLayerLimit.ProductId",
		// foreign affairs end
	};
		    collection = productLayerLimitService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProductLayerLimits",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProductLayerLimit" );
	    }
	    else if (navigation.equalsIgnoreCase ("delete")){
	    	result = deletePerformed (request, response );
	    }
	    else if (navigation.equalsIgnoreCase ("search") || navigation.equals("gosearch")){
	    	result = searchPerformed (request, response, "searchProductLayerLimit" );
	    }
	    else if (navigation.equalsIgnoreCase ("lookup") || navigation.equals("golookup")){
	    	result = searchPerformed (request, response, "lookupProductLayerLimit" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProductLayerLimit" );
	    }
