...
/**
 * Branch is a servlet controller for branch Table. All you have to do is to
...
// extends-
{
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			Branch res = branchService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.branch", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties
						.getMessage("fail.delete.branch", null, "fail", Locale
								.getDefault()));

			}
			
			LogUtil.log(logService, user, "Delete Branch", res.toString());
			result = searchPerformed(request, response, "searchBranch");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.branch", null, "fail", Locale.getDefault()));
			}
			/*
			...
			result.addObject("branch", object);
			result.addObject("index", index);
			result.addObject("searchtext", searchtext);
			result.addObject("searchby", searchby);

			LogUtil.log(logService, user, "view Detail Branch", object.toString());
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("branchName")) {
						vLikeP.add("branchName");
					...
					if (searchby.equalsIgnoreCase("branchHead")) {
						vLikeP.add("branchHead");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("branchFinanceHead")) {
						vLikeP.add("branchFinanceHead");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("branchActuaryHead")) {
						vLikeP.add("branchActuaryHead");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("branchMarketingHead")) {
						vLikeP.add("branchMarketingHead");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("branchClaimHead")) {
						vLikeP.add("branchClaimHead");
						vLikeQ.add(searchtext);
					}
				}
			...
			count = branchService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = branchService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			result.addObject("Branchs", collection);
			...
			LogUtil.log(logService, user, "Search Branch", "SearchText : " + searchtext + " SearchBy : " + searchby);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailBranch");
				...
				result = searchPerformed(request, response, "lookupBranch");
			} else {
				result = searchPerformed(request, response, "searchBranch");
			}
		} catch (Exception e) {
