...
*/
public class ReportRequestController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ReportRequest res = reportRequestService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.reportrequest",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.reportrequest",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchReportRequest");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.reportrequest",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("reportRequest", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    							    							    							    								...
			    							    							    							    							    								if (searchby.equalsIgnoreCase("tableOptions")){
					vLikeP.add("tableOptions");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("outputExtension")){
					vLikeP.add("outputExtension");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("urlReportResult")){
					vLikeP.add("urlReportResult");
					vLikeQ.add(searchtext);
				}
					...
					vLikeQ.add(searchtext);
							    							    								...
							    							    								if (searchby.equalsIgnoreCase("query")){
					vLikeP.add("query");
					vLikeQ.add(searchtext);
				}
							    			}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = reportRequestService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		...
		// foreign affairs end
		    ...
		    collection = reportRequestService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ReportRequests",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailReportRequest" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupReportRequest" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchReportRequest" );
	    }
