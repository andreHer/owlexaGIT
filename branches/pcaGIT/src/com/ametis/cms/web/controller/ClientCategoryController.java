...
*/
public class ClientCategoryController implements Controller
...
private ActivityLogService logService;
	...
	public void setClientCategoryService	(ClientCategoryService clientCategoryService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECLIENTCATEGORY");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECLIENTCATEGORY access");
				return errorResult;
				
			}
			ClientCategory res = clientCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.clientcategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.clientcategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchClientCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.clientcategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("clientCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("clientCategoryName")){
					vLikeP.add("clientCategoryName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("clientCategoryCode")){
					vLikeP.add("clientCategoryCode");
					vLikeQ.add(searchtext);
				}
			...
			count = clientCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = clientCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ClientCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailClientCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupClientCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchClientCategory" );
	    }
