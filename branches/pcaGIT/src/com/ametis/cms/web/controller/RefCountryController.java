...
/**
 * RefCountry is a servlet controller for ref_country Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class RefCountryController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			RefCountry res = refCountryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.refcountry",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.refcountry",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchRefCountry");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.refcountry",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("refCountry", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("countryCode")){
					vLikeP.add("countryCode");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("countryName")){
					vLikeP.add("countryName");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = refCountryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = refCountryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("RefCountrys",collection);
	...
	public ModelAndView lookupJsonPerformed(HttpServletRequest request,
		...
		try {
			...
			System.out.println("lookup json country dipanggil q : " + q);
			
			Collection<RefCountry> cities = refCountryService.searchCountry(q.toLowerCase());
			
			
			
			result.addObject("Countries", cities);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailRefCountry" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupRefCountry" );
	    }
	    else if (navigation.equalsIgnoreCase("lookupjson")){
	    	result = lookupJsonPerformed(request, response, "lookupJsonRefCountry");
	    }
	    else {
	    	result = searchPerformed (request, response, "searchRefCountry" );
	    }
