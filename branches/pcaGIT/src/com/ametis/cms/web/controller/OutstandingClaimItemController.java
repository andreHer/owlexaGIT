...
*/
public class OutstandingClaimItemController implements Controller
...
// extends- 
{
	...
	private OutstandingClaimItemService  oustandingClaimItemService;
...
private ActivityLogService logService;
	...
	public void setOutstandingClaimItemService	(OutstandingClaimItemService oustandingClaimItemService){
	this.oustandingClaimItemService = oustandingClaimItemService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEOUTSTANDINGCLAIM");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEOUTSTANDINGCLAIM access");
				return errorResult;
				
			}
		
			OutstandingClaimItem res = oustandingClaimItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.oustandingclaimitem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.oustandingclaimitem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchOustandingClaimItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.oustandingclaimitem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("oustandingClaimItem", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = oustandingClaimItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = oustandingClaimItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("OustandingClaimItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailOustandingClaimItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupOustandingClaimItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchOustandingClaimItem" );
	    }
