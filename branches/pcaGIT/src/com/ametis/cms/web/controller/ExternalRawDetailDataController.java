...
*/
public class ExternalRawDetailDataController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ExternalRawDetailData res = externalRawDetailDataService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.externalrawdetaildata",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.externalrawdetaildata",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchExternalRawDetailData");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.externalrawdetaildata",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("externalRawDetailData", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("data")){
					vLikeP.add("data");
			...
			count = externalRawDetailDataService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = externalRawDetailDataService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ExternalRawDetailDatas",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailExternalRawDetailData" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupExternalRawDetailData" );
	    }
            else if (navigation.equalsIgnoreCase ("list") ){
                Integer sessionId = WebUtil.getParameterInteger(request, "id");
	    	result = searchPerformed (request, response, "listExternalRawDetailData" );
