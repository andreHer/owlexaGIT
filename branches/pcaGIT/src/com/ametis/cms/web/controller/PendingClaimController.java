 ...
 */
public class PendingClaimController implements Controller
...
private ActivityLogService logService;
		...
		try {
			...
			if (!isUserAuthorized) {
				...
				errorResult.addObject("errorMessage",
						"You Are Not Authorized for DELETEPENDINGCLAIM access");
				return errorResult;

			}
			PendingClaim res = pendingClaimService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.pendingclaim", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.pendingclaim", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchPendingClaim");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.pendingclaim", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("pendingClaim", object);
	...
	public ModelAndView printPerformed(HttpServletRequest request,
		...
		try {
			...
			String[] required = {"Claim.MemberId","Claim.MemberId.ParentMember","Claim.DiagnosisId","Claim.MemberId.ClientId","Claim.MemberId.MemberGroupId"};
			...
			if (claim != null) {
				...
				Integer total = pendingClaimService.getTotal(null, null, param,
						value);
				Collection<PendingClaim> pendingClaims = pendingClaimService
						.search(null, null, param, value, 0, total);

				if (pendingClaims != null) {

					PendingClaim object = pendingClaims.iterator().next();

					if (object == null) {
						request.setAttribute("alert", alertProperties
								.getMessage("not.found.pendingclaim", null,
										"fail", Locale.getDefault()));
					}
					/*
					 * convention .. kalo mau nampilkan sebuah object pake
					 * BeanClass aja ya .. ga pake 's' -adhit
					 */
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup") || navigation.equalsIgnoreCase("golist") || navigation.equalsIgnoreCase("list")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("pendingSubject")) {
						vLikeP.add("pendingSubject");
			...
			count = pendingClaimService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = pendingClaimService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			if (collection.size() <= 0) {
				...
				collection = pendingClaimService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("PendingClaims", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				...
				result = printPerformed(request, response, "printPendingClaim");
			} else if (navigation.equalsIgnoreCase("search")
					|| navigation.equals("gosearch")) {
				result = searchPerformed(request, response,
						"searchPendingClaim");
			}else if (navigation.equalsIgnoreCase("extract")) {
				result = searchPerformed(request, response,
						"searchPendingClaim");
				breadcrumb = "<a href=\"batchclaim?navigation=detail&batchClaimId="+batchClaimId+"\" class=\"linkbreadcrumb\">Detail Batch Claim</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;Extract Claim";
				...
				result = searchPerformed(request, response,
					...
					|| subnavigation.equalsIgnoreCase("list") || navigation.equalsIgnoreCase("golist")) {
				result = searchPerformed(request, response, "listPendingClaim");
//				Integer batchClaimId = request.getParameter("batchClaimId");				
				breadcrumb = "<a href=\"batchclaim?navigation=detail&batchClaimId="+batchClaimId+"\" class=\"linkbreadcrumb\">Detail Batch Claim</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Pending Claim";
			} else {
				result = searchPerformed(request, response,
						"searchPendingClaim");
			}
