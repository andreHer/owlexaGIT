...
*/
public class QuestionaireItemController implements Controller
...

{
	...
	public void setQuestionaireItemService	(QuestionaireItemService questionaireItemService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			QuestionaireItem res = questionaireItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.questionaireitem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.questionaireitem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchQuestionaireItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.questionaireitem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("questionaireItem", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("questionaireSubject")){
					vLikeP.add("questionaireSubject");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = questionaireItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = questionaireItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("QuestionaireItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailQuestionaireItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupQuestionaireItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchQuestionaireItem" );
	    }
