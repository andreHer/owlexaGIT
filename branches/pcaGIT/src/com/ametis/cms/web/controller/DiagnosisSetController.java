...
/**
 * DiagnosisSet is a servlet controller for diagnosis_set Table. All you have to
 * do is to convert necessary data field to the named parameter
 */
public class DiagnosisSetController implements Controller
...
// extends-
{
	...
	public ModelAndView deletePerformed(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			DiagnosisSet res = diagnosisSetService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage("success.delete.diagnosisset", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert",
						alertProperties.getMessage("fail.delete.diagnosisset", null, "fail", Locale.getDefault()));

			}
			result = searchPerformed(request, response, "searchDiagnosisSet");
		} catch (Exception e) {
	...
	public ModelAndView lookupJsonPerformed(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ModelAndView result = null;

		try {

			result = new ModelAndView("lookupSetJson");
			String q = WebUtil.getParameterString(request, "q", "");

			String[] eqParam = { "deletedStatus" };
			Object[] eqValue = { Integer.valueOf(0) };

			String[] likeParam = { "diagnosisSetName" };
			Object[] likeValue = { q };

			Collection<DiagnosisSet> setList = diagnosisSetService.search(likeParam, likeValue, eqParam, eqValue, 0,
					15);
	...
	public ModelAndView detailPerformed(HttpServletRequest request, HttpServletResponse response, String location)
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert",
						alertProperties.getMessage("not.found.diagnosisset", null, "fail", Locale.getDefault()));
			}

			if (object != null) {
				String[] eqParam = { "diagnosisSetId.diagnosisSetId", "deletedStatus" };
				Object[] eqValue = { diagnosisSetId, 0 };
				int total = diagnosisSetDetailService.getTotal(null, null, eqParam, eqValue);
				Collection<DiagnosisSetDetail> diagnosisList = diagnosisSetDetailService.search(null, null, eqParam,
						eqValue, 0, total);

				result.addObject("DiagnosisSetDetails", diagnosisList);
			}

			result.addObject("diagnosisSet", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("diagnosisSetName")) {
						vLikeP.add("diagnosisSetName");
						vLikeQ.add(searchtext);
					}
					if (searchby.equalsIgnoreCase("diagnosisSetCode")) {
						vLikeP.add("diagnosisSetCode");
						vLikeQ.add(searchtext);
					}
			...
			count = diagnosisSetService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			String required[] = new String[] {
					// foreign affairs
					"DiagnosisSet.ClientId",
					// foreign affairs end
			};
			collection = diagnosisSetService.search(sLikeP, sLikeQ, sEqP, sEqQ, required, rowsetint,
					countSet.intValue());
			...
			result.addObject("DiagnosisSets", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				 ...
				 */
				result = detailPerformed(request, response, "detailDiagnosisSet");
				breadcrumb = "<a href=\"diagnosisset\" class=\"linkbreadcrumb\">Search Diagnosis Set</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;Detail Diagnosis Set";
				...
				result = searchPerformed(request, response, "lookupDiagnosisSet");
			} else {
				result = searchPerformed(request, response, "searchDiagnosisSet");
				breadcrumb = "<a href=\"diagnosisset\" class=\"linkbreadcrumb\">Search Diagnosis Set</a>";
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
