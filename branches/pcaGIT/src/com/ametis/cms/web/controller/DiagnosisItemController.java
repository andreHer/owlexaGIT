...
*/
public class DiagnosisItemController implements Controller
...
private ActivityLogService logService;
	...
	public void setDiagnosisItemService	(DiagnosisItemService diagnosisItemService){
	this.diagnosisItemService = diagnosisItemService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEDIAGNOSISITEM");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEDIAGNOSISITEM access");
				return errorResult;
				
			}
			DiagnosisItem res = diagnosisItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.diagnosisitem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.diagnosisitem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchDiagnosisItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.diagnosisitem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("diagnosisItem", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
			...
			count = diagnosisItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"DiagnosisItem.DiagnosisId",
				"DiagnosisItem.ItemId",
		// foreign affairs end
	};
		    collection = diagnosisItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("DiagnosisItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailDiagnosisItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupDiagnosisItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchDiagnosisItem" );
	    }
