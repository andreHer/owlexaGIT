...
*/
public class ExcessChargeItemController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEEXCESSITEM");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEEXCESSITEM access");
				return errorResult;
				
			}
			ExcessChargeItem res = excessChargeItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.excesschargeitem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.excesschargeitem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchExcessChargeItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.excesschargeitem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("excessChargeItem", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
			...
			count = excessChargeItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = excessChargeItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ExcessChargeItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailExcessChargeItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupExcessChargeItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchExcessChargeItem" );
	    }
