...
*/
public class DiagnosisMappingController implements Controller
...
// extends- 
{
	...
	private DiagnosisMappingService  diagnosisMappingService;
	...
	public void setDiagnosisMappingService	(DiagnosisMappingService diagnosisMappingService){
	this.diagnosisMappingService = diagnosisMappingService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			DiagnosisMapping res = diagnosisMappingService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.diagnosismapping",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.diagnosismapping",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchDiagnosisMapping");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.diagnosismapping",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("diagnosisMapping", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeQ.add(searchtext);
							    								...
							    								if (searchby.equalsIgnoreCase("clientDiagnosisCode")){
					vLikeP.add("clientDiagnosisCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = diagnosisMappingService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = diagnosisMappingService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("DiagnosisMappings",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailDiagnosisMapping" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupDiagnosisMapping" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchDiagnosisMapping" );
	    }
