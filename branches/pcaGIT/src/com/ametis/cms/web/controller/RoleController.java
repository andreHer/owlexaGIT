...
/**
 * Role is a servlet controller for role Table. All you have to do is to convert
...
private ActivityLogService logService;
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEROLE");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEROLE access");
				return errorResult;
				
			}
			Role res = roleService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.role", null, "success", Locale
								.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.role", null, "fail", Locale.getDefault()));

			}
			result = searchPerformed(request, response, "searchRole");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DETAILROLE");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DETAILROLE access");
				return errorResult;
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.role", null, "fail", Locale.getDefault()));
			}
			/*
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "SEARCHROLE");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for SEARCHROLE access");
				return errorResult;
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("roleName")) {
						vLikeP.add("roleName");
			...
			count = roleService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = roleService.search(sLikeP, sLikeQ, sEqP, sEqQ,
			...
			result.addObject("Roles", collection);
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		String breadcrumb = "<a href=\"role\" class=\"linkbreadcrumb\">Search Role</a>";
		
		try {
			if (navigation.equalsIgnoreCase("detail")) {
				result = detailPerformed(request, response, "detailRole");
				String roleId = request.getParameter("roleId");
				
				breadcrumb = "<a href=\"role?navigation=detail&roleId="+roleId+"\" class=\"linkbreadcrumb\">Detail Role</a>";
				...
				result = searchPerformed(request, response, "lookupRole");
			} else {
				result = searchPerformed(request, response, "searchRole");
			}
