...
*/
public class ProviderReviewController implements Controller
...

{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProviderReview res = providerReviewService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.providerreview",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.providerreview",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProviderReview");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.providerreview",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("providerReview", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = providerReviewService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = providerReviewService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProviderReviews",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProviderReview" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProviderReview" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProviderReview" );
	    }
