...
*/
public class GroupPackageController implements Controller
...
private ActivityLogService logService;
	...
	public void setGroupPackageService	(GroupPackageService groupPackageService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEGROUPPACKAGE");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEGROUPPACKAGE access");
				return errorResult;
				
			}
			GroupPackage res = groupPackageService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.grouppackage",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.grouppackage",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchGroupPackage");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.grouppackage",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("groupPackage", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = groupPackageService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"GroupPackage.GroupId",
				"GroupPackage.MemberGroupId",
				"GroupPackage.PackageId",
		// foreign affairs end
	};
		    collection = groupPackageService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("GroupPackages",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailGroupPackage" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupGroupPackage" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchGroupPackage" );
	    }
