...
/**
 * GroupProduct is a servlet controller for group_product Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class GroupProductController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETEGROUPPRODUCT");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETEGROUPPRODUCT access");
				return errorResult;
				
			}
			GroupProduct res = groupProductService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.groupproduct",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.groupproduct",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchGroupProduct");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.groupproduct",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("groupProduct", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = groupProductService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"GroupProduct.GroupId",
				"GroupProduct.ProductId",
				"GroupProduct.MemberGroupId",
		// foreign affairs end
	};
		    collection = groupProductService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("GroupProducts",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailGroupProduct" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupGroupProduct" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchGroupProduct" );
	    }
