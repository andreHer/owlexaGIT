...
*/
public class ClausulCategoryController implements Controller
...

{
	...
	private ClausulCategoryService  clausulCategoryService;
...
private ActivityLogService logService;
	...
	public void setClausulCategoryService	(ClausulCategoryService clausulCategoryService){
	this.clausulCategoryService = clausulCategoryService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
						...
						boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETECLAUSULCATEGORY");
						
						if (!isUserAuthorized){
							
							ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
							errorResult.addObject("errorMessage", "You Are Not Authorized for DELETECLAUSULCATEGORY access");
							return errorResult;
							
						}
			ClausulCategory res = clausulCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.clausulcategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.clausulcategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchClausulCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.clausulcategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("clausulCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("clausulCategoryName")){
					vLikeP.add("clausulCategoryName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("clausulCategoryCode")){
					vLikeP.add("clausulCategoryCode");
					vLikeQ.add(searchtext);
				}
		   ...
		   vEqQ.add(Integer.valueOf(0));
			...
			count = clausulCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = clausulCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ClausulCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailClausulCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupClausulCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchClausulCategory" );
	    }
