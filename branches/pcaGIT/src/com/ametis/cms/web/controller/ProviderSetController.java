...
*/
public class ProviderSetController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProviderSet res = providerSetService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.providerset",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.providerset",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProviderSet");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.providerset",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("providerSet", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("providerSetName")){
					vLikeP.add("providerSetName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("providerSetCode")){
					vLikeP.add("providerSetCode");
					vLikeQ.add(searchtext);
				}
			...
			count = providerSetService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = providerSetService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    result.addObject("ProviderSets",collection);
...
*/
	...
	try {
	    ...
	    */
	    	...
	    	result = detailPerformed (request, response, "detailProviderSet" );
	    	breadcrumb = "<a href=\"providerset?navigation="+navigation+"&index=1&providerSetId="+providerSetId+"\" class=\"linkbreadcrumb\">Detail Provider Profile</a>";
	    }
	    	...
	    	result = searchPerformed (request, response, "searchProviderSet" );
	    	breadcrumb = "<a href=\"providerset?navigation=search&searchtext=&searchby=&index=1\" class=\"linkbreadcrumb\">Search Provider Set Profile</a>";
	    }
	    else if (navigation.equalsIgnoreCase ("lookup") || navigation.equals("golookup")){
	    	result = searchPerformed (request, response, "lookupProviderSet" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProviderSet" );
	    	breadcrumb = "<a href=\"providerset?navigation=search&searchtext=&searchby=&index=1\" class=\"linkbreadcrumb\">Search Provider Set Profile</a>";
