...
*/
public class PaymentDocumentController implements Controller
...

{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			PaymentDocument res = paymentDocumentService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.paymentdocument",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.paymentdocument",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPaymentDocument");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.paymentdocument",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("paymentDocument", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = paymentDocumentService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = paymentDocumentService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("PaymentDocuments",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPaymentDocument" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPaymentDocument" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPaymentDocument" );
	    }
