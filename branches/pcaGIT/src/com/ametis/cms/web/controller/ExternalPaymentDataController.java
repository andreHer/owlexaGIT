...
*/
public class ExternalPaymentDataController implements Controller
...
private ActivityLogService logService;
	...
	public void setExternalPaymentDataService	(ExternalPaymentDataService externalPaymentDataService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ExternalPaymentData res = externalPaymentDataService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.externalpaymentdata",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.externalpaymentdata",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchExternalPaymentData");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.externalpaymentdata",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("externalPaymentData", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("policyHolder")){
					vLikeP.add("policyHolder");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = externalPaymentDataService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = externalPaymentDataService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ExternalPaymentDatas",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailExternalPaymentData" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupExternalPaymentData" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchExternalPaymentData" );
	    }
