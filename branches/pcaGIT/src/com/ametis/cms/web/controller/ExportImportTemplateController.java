...
*/
public class ExportImportTemplateController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ExportImportTemplate res = exportImportTemplateService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.exportimporttemplate",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.exportimporttemplate",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchExportImportTemplate");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.exportimporttemplate",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("exportImportTemplate", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeQ.add(searchtext);
							    								...
							    								if (searchby.equalsIgnoreCase("templateMapping")){
					vLikeP.add("templateMapping");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = exportImportTemplateService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = exportImportTemplateService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ExportImportTemplates",collection);
...
*/
	...
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailExportImportTemplate" );
	    }
	    	...
	    	result = searchPerformed (request, response, "searchExportImportTemplate" );
	    	breadcrumb = "<a href=\"exportimporttemplate\" class=\"linkbreadcrumb\">Search Export Import Template</a>";
	    }
	    else if (navigation.equalsIgnoreCase ("lookup") || navigation.equals("golookup")){
	    	result = searchPerformed (request, response, "lookupExportImportTemplate" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchExportImportTemplate" );
	    	breadcrumb = "<a href=\"exportimporttemplate\" class=\"linkbreadcrumb\">Search Export Import Template</a>";
