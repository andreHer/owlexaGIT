...
/**
 * CardType is a servlet controller for card_type Table.
...

{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			CardType res = cardTypeService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.cardtype",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.cardtype",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchCardType");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.cardtype",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("cardType", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("cardTypeName")){
					vLikeP.add("cardTypeName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("cardTypeCode")){
					vLikeP.add("cardTypeCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = cardTypeService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = cardTypeService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("CardTypes",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailCardType" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupCardType" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchCardType" );
	    }
