...
*/
public class PolicyBillingRateController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			PolicyBillingRate res = policyBillingRateService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.policybillingrate",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.policybillingrate",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchPolicyBillingRate");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.policybillingrate",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("policyBillingRate", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = policyBillingRateService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"PolicyBillingRate.ItemId",
				"PolicyBillingRate.PolicyId",
		// foreign affairs end
	};
		    collection = policyBillingRateService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("PolicyBillingRates",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailPolicyBillingRate" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupPolicyBillingRate" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchPolicyBillingRate" );
	    }
