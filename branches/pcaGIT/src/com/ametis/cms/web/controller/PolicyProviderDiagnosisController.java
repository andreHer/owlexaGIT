...
/**
 * PolicyProviderDiagnosis is a servlet controller for policy_provider_diagnosis
 * Table. All you have to do is to convert necessary data field to the named
 * parameter
 */
public class PolicyProviderDiagnosisController implements Controller
...

{
	...
	private PolicyProviderDiagnosisService policyProviderDiagnosisService;
	...
	public void setPolicyProviderDiagnosisService(
			PolicyProviderDiagnosisService policyProviderDiagnosisService) {
		this.policyProviderDiagnosisService = policyProviderDiagnosisService;
	}
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer policyProviderDiagnosisId = WebUtil.getParameterInteger(
					request, "policyProviderDiagnosisId");
			...
			PolicyProviderDiagnosis res = policyProviderDiagnosisService
					.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.policyproviderdiagnosis", null,
						"success", Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.policyproviderdiagnosis", null, "fail",
						Locale.getDefault()));

			}
			result = searchPerformed(request, response,
					"searchPolicyProviderDiagnosis");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			java.io.Serializable pkey = policyProviderDiagnosisId;
			PolicyProviderDiagnosis object = policyProviderDiagnosisService
					.get(pkey);

			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.policyproviderdiagnosis", null, "fail",
						Locale.getDefault()));
			}
			/*
			...
			result.addObject("policyProviderDiagnosis", object);
	...
	public ModelAndView searchPerformed(HttpServletRequest request,
		...
		try {
			...
			if (policyProviderId != null){
				vEqP.add("policyProviderId.policyProviderId");
				vEqQ.add(policyProviderId);
			}
			...
			count = policyProviderDiagnosisService.getTotal(sLikeP, sLikeQ,
					sEqP, sEqQ);
			...
			String required[] = new String[] {
					// foreign affairs
					"PolicyProviderDiagnosis.DiagnosisId",
					"PolicyProviderDiagnosis.PolicyProviderId",
					
					"PolicyProviderDiagnosis.PolicyProviderId.PolicyId",
					"PolicyProviderDiagnosis.PolicyProviderId.PolicyId.ClientId",
			// foreign affairs end
			};
			collection = policyProviderDiagnosisService.search(sLikeP, sLikeQ,
			...
			if (collection.size() <= 0) {
				...
				collection = policyProviderDiagnosisService.search(sLikeP,
			...
			result.addObject("PolicyProviderDiagnosiss", collection);

			request.setAttribute("policyProviderId", policyProviderId);
