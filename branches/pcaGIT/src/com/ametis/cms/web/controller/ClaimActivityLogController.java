...
*/
public class ClaimActivityLogController implements Controller
...

{
	...
	public void setClaimActivityLogService	(ClaimActivityLogService claimActivityLogService){
	this.claimActivityLogService = claimActivityLogService;
	}
	public ClaimActivityLogService getClaimActivityLogService (){
	return this.claimActivityLogService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ClaimActivityLog res = claimActivityLogService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.claimactivitylog",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.claimactivitylog",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchClaimActivityLog");
		}
		catch (Exception e){
	    ...
	    try {
			...
			*/
			...
			result = new ModelAndView (location);
						java.io.Serializable pkey = id;
				        ClaimActivityLog object = claimActivityLogService.get (pkey);

			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.claimactivitylog",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("claimActivityLog", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeQ.add(searchtext);
							    								...
							    								if (searchby.equalsIgnoreCase("beforeActionClaimData")){
					vLikeP.add("beforeActionClaimData");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("afterActionClaimData")){
					vLikeP.add("afterActionClaimData");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("ipAddress")){
					vLikeP.add("ipAddress");
					vLikeQ.add(searchtext);
				}
							    			}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = claimActivityLogService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = claimActivityLogService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ClaimActivityLogs",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailClaimActivityLog" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupClaimActivityLog" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchClaimActivityLog" );
	    }
