 ...
 */
public class ExcessReminderController implements Controller
...
// extends-
{
	...
	public void setExcessReminderService(
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			Integer excessReminderId = WebUtil.getParameterInteger(request,
					"excessReminderId");
			...
			if (!isUserAuthorized) {
				...
				errorResult
						.addObject("errorMessage",
								"You Are Not Authorized for DELETEEXCESSREMINDER access");
				return errorResult;

			}
			ExcessReminder res = excessReminderService.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.excessreminder", null, "success",
						Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.excessreminder", null, "fail", Locale
								.getDefault()));

			}
			result = searchPerformed(request, response, "searchExcessReminder");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.excessreminder", null, "fail", Locale
								.getDefault()));
			}
			/*
			...
			result.addObject("excessReminder", object);
	...
	public ModelAndView printPerformed(HttpServletRequest request,
		...
		try {
			...
			String location =  "printExcessReminder";
			result = new ModelAndView(location);
			java.io.Serializable pkey = excessReminderId;
			...
			result.addObject("member", object.getExcessChargeId().getMemberId());
			result.addObject("policyHolder", object.getExcessChargeId().getMemberId().getParentMember());
			result.addObject("excessCharge",object.getExcessChargeId());
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("reminderRemarks")) {
						vLikeP.add("reminderRemarks");
					...
					if (searchby.equalsIgnoreCase("reminderBy")) {
						vLikeP.add("reminderBy");
						vLikeQ.add(searchtext);
					}
				}
			...
			count = excessReminderService.getTotal(sLikeP, sLikeQ, sEqP, sEqQ);
			...
			collection = excessReminderService.search(sLikeP, sLikeQ, sEqP,
			...
			result.addObject("ExcessReminders", collection);
