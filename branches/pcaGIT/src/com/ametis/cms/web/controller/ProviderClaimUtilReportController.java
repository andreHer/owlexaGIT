...
/**
 * ProviderClaimUtilReport is a servlet controller for
 * provider_claim_util_report Table. All you have to do is to convert necessary
 * data field to the named parameter
 */
public class ProviderClaimUtilReportController implements Controller
...
// extends-
{
	...
	private ProviderClaimUtilReportService providerClaimUtilReportService;
...
private ActivityLogService logService;
	...
	public void setProviderClaimUtilReportService(
			ProviderClaimUtilReportService providerClaimUtilReportService) {
		this.providerClaimUtilReportService = providerClaimUtilReportService;
	}
	...
	public ModelAndView deletePerformed(HttpServletRequest request,
		...
		try {
			...
			ProviderClaimUtilReport res = providerClaimUtilReportService
					.delete(pkey, user);

			if (res != null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"success.delete.providerclaimutilreport", null,
						"success", Locale.getDefault()));
			} else {
				request.setAttribute("alert", alertProperties.getMessage(
						"fail.delete.providerclaimutilreport", null, "fail",
						Locale.getDefault()));

			}
			result = searchPerformed(request, response,
					"searchProviderClaimUtilReport");
		} catch (Exception e) {
	...
	public ModelAndView detailPerformed(HttpServletRequest request,
		...
		try {
			...
			java.io.Serializable pkey = id;
			ProviderClaimUtilReport object = providerClaimUtilReportService
					.get(pkey);

			if (object == null) {
				request.setAttribute("alert", alertProperties.getMessage(
						"not.found.providerclaimutilreport", null, "fail",
						Locale.getDefault()));
			}
			/*
			...
			result.addObject("providerClaimUtilReport", object);
		...
		try {
			...
			if (navigation.equals("gosearch") || navigation.equals("golookup")) {
				...
				if (searchby != null) {
					...
					if (searchby.equalsIgnoreCase("providerPeriode")) {
						vLikeP.add("providerPeriode");
						vLikeQ.add(searchtext);
					}
				}
			...
			count = providerClaimUtilReportService.getTotal(sLikeP, sLikeQ,
					sEqP, sEqQ);
			...
			collection = providerClaimUtilReportService.search(sLikeP, sLikeQ,
			...
			if (collection.size() <= 0) {
				...
				collection = providerClaimUtilReportService.search(sLikeP,
			...
			result.addObject("ProviderClaimUtilReports", collection);
