...
*/
public class QuestionaireController implements Controller
...

{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			Questionaire res = questionaireService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.questionaire",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.questionaire",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchQuestionaire");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.questionaire",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("questionaire", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("questionaireName")){
					vLikeP.add("questionaireName");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = questionaireService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = questionaireService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Questionaires",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailQuestionaire" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupQuestionaire" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchQuestionaire" );
	    }
