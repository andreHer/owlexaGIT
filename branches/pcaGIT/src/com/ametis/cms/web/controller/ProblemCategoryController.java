...
/**
 * ProblemCategory is a servlet controller for problem_category Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class ProblemCategoryController implements Controller
...

{
	...
	public void setProblemCategoryService	(ProblemCategoryService problemCategoryService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ProblemCategory res = problemCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.problemcategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.problemcategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchProblemCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.problemcategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("problemCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("problemCategoryName")){
					vLikeP.add("problemCategoryName");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = problemCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = problemCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ProblemCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailProblemCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupProblemCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchProblemCategory" );
	    }
