...
*/
public class MedicineCategoryController implements Controller
...

{
	...
	public void setMedicineCategoryService	(MedicineCategoryService medicineCategoryService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			MedicineCategory res = medicineCategoryService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.medicinecategory",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.medicinecategory",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchMedicineCategory");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.medicinecategory",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("medicineCategory", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("medicineCategoryName")){
					vLikeP.add("medicineCategoryName");
					vLikeQ.add(searchtext);
				}
							    								if (searchby.equalsIgnoreCase("medicineCategoryCode")){
					vLikeP.add("medicineCategoryCode");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = medicineCategoryService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = medicineCategoryService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("MedicineCategorys",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailMedicineCategory" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupMedicineCategory" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchMedicineCategory" );
	    }
