...
*/
public class CardStockUsageController implements Controller
...
// extends- 
{
	...
	private CardStockUsageService  cardStockUsageService;
	...
	public void setCardStockUsageService	(CardStockUsageService cardStockUsageService){
	this.cardStockUsageService = cardStockUsageService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			CardStockUsage res = cardStockUsageService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.cardstockusage",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.cardstockusage",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchCardStockUsage");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.cardstockusage",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("cardStockUsage", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup") || navigation.equals("listclient")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("cardPrintBatchNumber")){
					vLikeP.add("cardPrintBatchNumber");
			...
			count = cardStockUsageService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = cardStockUsageService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("CardStockUsages",collection);
...
*/
	...
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailCardStockUsage" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupCardStockUsage" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchCardStockUsage" );
	    	String clientId = request.getParameter("clientId");
			breadcrumb = "<a href=\"client?navigation=detail&clientId="+clientId+"\" class=\"linkbreadcrumb\">Detail Client</a> &nbsp;&nbsp;<b>&gt</b> &nbsp;&nbsp;List Client Card Stock Usage";
	    }
    }
    catch (Exception e){
    	e.printStackTrace();
