...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			Refund res = refundService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.refund",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.refund",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchRefund");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.refund",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("refund", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    								...
			    							    								if (searchby.equalsIgnoreCase("refundNumber")){
					vLikeP.add("refundNumber");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = refundService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = refundService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("Refunds",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailRefund" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupRefund" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchRefund" );
	    }
