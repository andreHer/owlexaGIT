...
/**
 * RefundItem is a servlet controller for refund_item Table.
 * All you have to do is to convert necessary data field to the named parameter
*/
public class RefundItemController implements Controller
...
// extends- 
{
	...
	public void setRefundItemService	(RefundItemService refundItemService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			RefundItem res = refundItemService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.refunditem",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.refunditem",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchRefundItem");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.refunditem",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("refundItem", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = refundItemService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"RefundItem.MemberImportId",
				"RefundItem.RefundId",
				"RefundItem.MemberId",
		// foreign affairs end
	};
		    collection = refundItemService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("RefundItems",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailRefundItem" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupRefundItem" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchRefundItem" );
	    }
