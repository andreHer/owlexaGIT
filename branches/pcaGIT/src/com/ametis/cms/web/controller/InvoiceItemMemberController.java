...
*/
public class InvoiceItemMemberController implements Controller
...
// extends- 
{
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			InvoiceItemMember res = invoiceItemMemberService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.invoiceitemmember",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.invoiceitemmember",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchInvoiceItemMember");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.invoiceitemmember",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("invoiceItemMember", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = invoiceItemMemberService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
				"InvoiceItemMember.InvoiceItemId",
				"InvoiceItemMember.MemberId",
		// foreign affairs end
	};
		    collection = invoiceItemMemberService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("InvoiceItemMembers",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailInvoiceItemMember" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupInvoiceItemMember" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchInvoiceItemMember" );
	    }
