...
*/
public class ServiceDiagnosisController implements Controller
...

{
	...
	private ServiceDiagnosisService  serviceDiagnosisService;
...
private ActivityLogService logService;
	...
	public void setServiceDiagnosisService	(ServiceDiagnosisService serviceDiagnosisService){
	this.serviceDiagnosisService = serviceDiagnosisService;
	}
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			boolean isUserAuthorized = securityService.isUserAuthorized(user, "DELETESERVICEDIAGNOSIS");
			
			if (!isUserAuthorized){
				
				ModelAndView errorResult = new ModelAndView(new RedirectView("errorpage"));	errorResult.addObject("errorType","accessDenied");			
				errorResult.addObject("errorMessage", "You Are Not Authorized for DELETESERVICEDIAGNOSIS access");
				return errorResult;
				
			}
			ServiceDiagnosis res = serviceDiagnosisService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.servicediagnosis",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.servicediagnosis",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchServiceDiagnosis");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.servicediagnosis",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("serviceDiagnosis", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		  ...
		  vEqQ.add(new Integer(0));
			...
			count = serviceDiagnosisService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = serviceDiagnosisService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ServiceDiagnosiss",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailServiceDiagnosis" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupServiceDiagnosis" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchServiceDiagnosis" );
	    }
