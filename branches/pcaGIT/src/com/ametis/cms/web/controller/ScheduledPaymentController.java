...
*/
public class ScheduledPaymentController implements Controller
...
private ActivityLogService logService;
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			ScheduledPayment res = scheduledPaymentService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.scheduledpayment",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.scheduledpayment",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchScheduledPayment");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.scheduledpayment",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("scheduledPayment", object);
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
			    							    							    								...
			    							    							    								if (searchby.equalsIgnoreCase("number")){
					vLikeP.add("number");
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = scheduledPaymentService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = scheduledPaymentService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("ScheduledPayments",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailScheduledPayment" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupScheduledPayment" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchScheduledPayment" );
	    }
