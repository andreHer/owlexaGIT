...
*/
public class OutstandingClaimController implements Controller
...
private ActivityLogService logService;
	...
	public void setOutstandingClaimService	(OutstandingClaimService outstandingClaimService){
	    ...
	    ModelAndView result = null;
 	    ...
 	    try {
			...
			*/
			...
			OutstandingClaim res = outstandingClaimService.delete (pkey,user);

			if (res!=null){
				request.setAttribute ("alert", alertProperties.getMessage ("success.delete.outstandingclaim",null,"success",Locale.getDefault()));
			}
			else {
				request.setAttribute ("alert", alertProperties.getMessage ("fail.delete.outstandingclaim",null,"fail",Locale.getDefault()));

			}
			result = searchPerformed (request, response, "searchOutstandingClaim");
		}
		catch (Exception e){
	...
	public ModelAndView detailPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
	    ...
	    try {
			...
			*/
			...
			if (object == null){
				request.setAttribute ("alert", alertProperties.getMessage ("not.found.outstandingclaim",null,"fail",Locale.getDefault()));
			}
			/*
			...
			*/
		    ...
		    result.addObject ("outstandingClaim", object);
	...
	public ModelAndView searchPerformed (HttpServletRequest request, HttpServletResponse response, String location) throws Exception {
		...
		try {
		    ...
		    if (navigation.equals("gosearch") || navigation.equals("golookup")){
			    ...
			    */
					...
					vLikeQ.add(searchtext);
							    								...
							    								if (searchby.equalsIgnoreCase("serviceCode")){
					vLikeP.add("serviceCode");
					vLikeQ.add(searchtext);
				}
							    							    							    								if (searchby.equalsIgnoreCase("covertedBy")){
					vLikeP.add("covertedBy");
					vLikeQ.add(searchtext);
				}
		  ...
		  //  vEqQ.add(new Integer(0));
			...
			count = outstandingClaimService.getTotal (sLikeP,sLikeQ,sEqP,sEqQ);
	...
	// foreign affairs
		// foreign affairs end
		    ...
		    collection = outstandingClaimService.search (sLikeP,sLikeQ,sEqP,sEqQ,
		    ...
		    */
		    ...
		    result.addObject("OutstandingClaims",collection);
...
*/
	try {
	    ...
	    */
	    	result = detailPerformed (request, response, "detailOutstandingClaim" );
	    }
	    	...
	    	result = searchPerformed (request, response, "lookupOutstandingClaim" );
	    }
	    else {
	    	result = searchPerformed (request, response, "searchOutstandingClaim" );
	    }
