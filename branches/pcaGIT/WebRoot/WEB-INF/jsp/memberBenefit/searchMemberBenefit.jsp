...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tbody>
		<tr>
			<td nowrap="nowrap">
				<h3>
					<img src="images/h3Arrow.gif" alt="Call Search" border="0">
					&nbsp;Search Member Benefit
				</h3>
			</td>
			<td width="100%">
...
<div class="table_container">
	...
	<div id="toolbar">
		...
		<div class="right">
			<a href="#" class="btn_search" onclick="Effect.toggle('search_float','blind'); return false;"> <c:out value="${SEARCH}" /></a>
		</div>
		...
		<div id="search_float" <%=nampak%>>
			...
			<form name=form1 onSubmit="cari()">
				...
				<select name="searchby" class="inputbox">
					...
					<option name="createdBy" <c:if test="${searchby eq \"createdBy\"}">selected="true"</c:if>>
						Created By
					</option>
					<option name="deletedBy" <c:if test="${searchby eq \"deletedBy\"}">selected="true"</c:if>>
						Deleted By
					</option>
					<option name="modifiedBy" <c:if test="${searchby eq \"modifiedBy\"}">selected="true"</c:if>>
						Modified By
					</option>
				</select>
				<BR />
...
%>
		<c:forEach items="${MemberBenefits}" var="memberBenefit" varStatus="status">
			<%	if (i % 2 == 0) {
			rowclass = "col1";
		} else if (i % 2 != 0) {
			rowclass= "col2";
	...
	%>
			...
			<tr class="<%=rowclass%>" onmouseover="this.className='highlight'" onmouseout="this.className='<%=rowclass%>'" height="20">
				...
				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
					...
					<c:out value="${memberBenefit.createdBy}" />
					
				</td>
				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
					
					<c:out value="${memberBenefit.deletedBy}" />
					
				</td>
				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
					
					<c:out value="${memberBenefit.modifiedBy}" />
					
				</td>

				<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" align="center">
					<!-- ini default edit table for each data -->
					<a href="javascript:detil('<c:out value="${memberBenefit.memberBenefitId}" />')"> <img src="images/icon_view.gif" class="action_icon" alt="View" title="View"></a>

					<!-- ini default edit table for each data -->
					<a href="javascript:ubah('<c:out value="${memberBenefit.memberBenefitId}" />')"> <img src="images/icon_edit.gif" class="action_icon" alt="Edit" title="Edit"></a>

					<!-- ini default delete table for each data -->
					<a href="javascript:hapus('<c:out value="${memberBenefit.memberBenefitId}" />')"> <img src="images/icon_delete.gif" class="action_icon" alt="Delete" title="Delete"></a>
...
function hapus (idx){
	...
	if (delConfirm){
		...
		document.form1.memberBenefitId.value = idx;
