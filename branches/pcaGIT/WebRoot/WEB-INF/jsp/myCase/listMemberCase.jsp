...
<table>
	<tbody>
		<tr>
			<td nowrap="nowrap"><h3>
					<img src="images/h3Arrow.gif" border="0">&nbsp;List Member
					Case
				</h3></td>
			<td width="100%"><img src="images/blank.gif" height="1"
				width="1"></td>
			<c:if test="${theUser.userType eq 2 }">
				<td align="right"><input title="Error Log" name="errorLog"
					id="errorLog" value=" Error Log " class="errorLog" type="button"
					onClick="javascript:printErrorLog()">
				</td>
				<td align="right"><input title="Add Error Log"
					name="addErrorLog" value=" Add Error Log " class="errorLog"
					type="button" onClick="javascript:adderrorlog()">
				</td>
			</c:if>
		</tr>
	</tbody>
...
<form name="form1" action="case" method="POST">
	<input type="hidden" name="navigation" value="listmember"> <input
		type="hidden" name="subnavigation"
		value="<c:out value="${subnavigation}" />"> <input
		type="hidden" name="providerId"
		value="<c:out value="${providerId }" />"> <input type="hidden"
		name="currentnavigation" value="listmember"> <input
		type="hidden" name="memberId" value="<c:out value="${memberId}" />" />
	<input type="hidden" name="arah" value=""> <input type="hidden"
		name="index" value="<c:out value="${index}" />"> <input
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%"
		...
		<tbody>
			<tr>
				<td nowrap="nowrap"><h3>
						<img src="images/h3Arrow.gif" border="0">&nbsp;Search Member
						Case
					</h3></td>
				<td width="100%"><img src="images/blank.gif" height="1"
					width="1"></td>
						...
						<tbody>
							<tr>
								...
								<td class="dataLabel" nowrap="nowrap">Search Category:
									...
									&nbsp;&nbsp; <select name="searchby" class="inputbox">
										...
										<option value="providerName"
											<c:if test="${searchby eq \"providerName\"}">selected="true"</c:if>>
											Hospital</option>
										<option value="category"
											<c:if test="${searchby eq \"category\"}">selected="true"</c:if>>
								...
								<td class="dataLabel"><input title="Search [Alt+Shift+Q]"
									...
									name="button" value="Search" type="button">
								</td>
							</tr>
			...
			<c:forEach items="${Cases}" var="myCase" varStatus="status">
				...
				<tr height="20">
					...
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><a
						href="case?navigation=detail&url=member&caseId=<c:out value="${myCase.caseId}" />"
						class="linkDetail"><c:out value="${myCase.caseNumber}" /> </a>
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out value="${myCase.memberId.firstName}" />
						<c:out value="${myCase.memberId.lastName}" />
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out value="${myCase.providerId.providerName}" />
					...
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out
							value="${myCase.diagnosis1Id.diagnosisName}" />
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out
					...
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out value="${myCase.claimId.admissionDate}" />

					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top"><c:out value="${myCase.claimId.dischargeDate}" />
...
function go(){
	...
	function printErrorLog() {
		window
				.open(
						"firstcall?navigation=searchmembererrorlog&memberId=<c:out value="${member.memberId}" />",
						"Search",
