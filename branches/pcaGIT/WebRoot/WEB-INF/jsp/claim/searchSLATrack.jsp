...
<form name="form1" action="claim" method="POST">
	<input type="hidden" name="navigation" value="searchsla">
						...
						<tbody>
							<tr>
								...
								<td class="dataLabel" nowrap="nowrap">
									...
									<input size="20" name="searchtext" class="dataField"
								...
								<td class="dataLabel" nowrap="nowrap">
									...
									<select name="searchby" class="inputbox">
										<option value="memberGroupName"
											<c:if test="${searchby eq \"memberGroupName\"}">selected="true"</c:if>>
											Member Group Name
										</option>
										<option value="claimNumber"
											<c:if test="${searchby eq \"claimNumber\"}">selected="true"</c:if>>
											Claim Number
										</option>
										...
										<option value="clientName"
											<c:if test="${searchby eq \"clientName\"}">selected="true"</c:if>>
											Client Name
										</option>
										...
										<option value="policyNumber"
											<c:if test="${searchby eq \"policyNumber\"}">selected="true"</c:if>>
											Policy Number
										</option>
										<option value="receivedDate"
											<c:if test="${searchby eq \"receivedDate\"}">selected="true"</c:if>>
											Received Date
										</option>
								...
								<td class="dataLabel" nowrap="nowrap">
									...
									<select name="status">
										...
										<option value="9" <c:if test="${status eq 9 }">selected</c:if>>
											Approved
										</option>
										<option value="8" <c:if test="${status eq 8 }">selected</c:if>>
											Benefit Checked
										</option>
										...
										<option value="10"
											<c:if test="${status eq 10 }">selected</c:if>>
											Pending
										</option>
									</select>
								...
								<td class="dataLabel">
									<input title="Search [Alt+Shift+Q]" accesskey="Q"
										class="ui-button ui-widget ui-state-default ui-corner-all ui-state-focus" name="button" value="Search" type="submit">
							...
							<tr>
								...
								<td class="dataLabel" nowrap="nowrap">
									...
									<input name="minimum_date" id="jscal_field" tab="1"
										maxlength="10" size="11" readonly="readonly"
										value="<c:out value="${minDate}" />" type="text">
									<img src="images/jscalendar.gif" alt="Enter Date"
										id="jscal_trigger" align="absmiddle" height="18" width="18">
    					...
    					Calendar.setup({
        					...
        					singleClick    :    true
								...
								<td class="dataLabel" nowrap="nowrap">
									...
									<input name="maximum_date" id="jscal_field_max" tabindex="1"
										maxlength="10" size="11" readonly="readonly"
										value="<c:out value="${maxDate}" />" type="text">
									<img src="images/jscalendar.gif" alt="Enter Date"
										id="jscal_max_trigger" align="absmiddle" height="18"
										width="18">
        					...
        					singleClick    :    true
								...
								<td class="dataLabel" nowrap="nowrap">
									...
									<select name="claimType">
										...
										<option value="1"
											<c:if test="${claimType eq 1 }">selected</c:if>>
											REIMBURSEMENT
										</option>
										<option value="2"
											<c:if test="${claimType eq 2 }">selected</c:if>>
											CASHLESS
										</option>
							...
							<tr>
								...
								<td class="dataLabel" nowrap="nowrap">
									...
									<select name="serviceType">
										...
										<option value="1"
											<c:if test="${serviceType eq 1 }">selected</c:if>>
											INPATIENT
										</option>
										<option value="2"
											<c:if test="${serviceType eq 2 }">selected</c:if>>
											OUTPATIENT
										</option>
										<option value="3"
											<c:if test="${serviceType eq 3 }">selected</c:if>>
											MATERNITY
										</option>
										<option value="4"
											<c:if test="${serviceType eq 4 }">selected</c:if>>
											DENTAL
										</option>
										<option value="5"
											<c:if test="${serviceType eq 5 }">selected</c:if>>
											OPTICAL
										</option>
									</select>
	...
	<table class="listView" cellspacing="0" cellpadding="0" width="100%">
		<tbody>
			...
			<c:forEach items="${Claims}" var="claim" varStatus="status">
				...
				<tr height="20">
					<td class="oddListRowS1" align="center" bgcolor="#e7f0fe"
						nowrap="nowrap" valign="top"><%=i%></td>

					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top">
						<a
							href="claim?navigation=detail&claimId=<c:out value="${claim.claimId}" />"
							class="linkDetail"><c:out value="${claim.claimNumber}" />
						</a>
					</td>
					<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap"
						valign="top">
			...
			<tr height="20">
				...
				<td scope="col" class="listViewThS1" style="text-align: right;"
					nowrap="nowrap" width="15%" align="right">
				</td>

				<td scope="col" class="listViewThS1" style="text-align: right;"
					nowrap="nowrap" width="10%" align="right">
				</td>
