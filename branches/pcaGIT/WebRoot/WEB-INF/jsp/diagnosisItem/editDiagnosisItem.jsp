...
<form action="diagnosisitem-form" method="POST"  name="form1" id="form_layout">
...
if (alert != null && !alert.trim().equals("")) {%>
	...
	<spring:bind path="diagnosisItemForm.diagnosisItemId">
...
<input type="hidden" name="navigation" value="">
			...
			<tr>
				<td class="dataLabel"> Created Time : </td>				
						...
						<td class="dataField">
			<spring:bind path="diagnosisItemForm.createdTime">
	    ...
	    <td class="dataField"></td>
			...
			<td class="dataField">
			<spring:bind path="diagnosisItemForm.createdBy">
	    ...
	    <td class="dataField"></td>
			...
			<tr>
				<td class="dataLabel"> Deleted Time : </td>				
						...
						<td class="dataField">
			<spring:bind path="diagnosisItemForm.deletedTime">
					<input type="text" size="35" name="<c:out value="${status.expression}" />" value="<c:out value="${status.value}" />" id="<c:out value="${status.value}"/>" maxlength="19">
					<img src="images/jscalendar.gif" alt="Enter Date" id="<c:out value="${status.expression}"/>_trigger" align="absmiddle" height="20" width="20">
					<script type="text/javascript">
    					Calendar.setup({
	    ...
	    <td class="dataField"></td>
			...
			<td class="dataField">
			<spring:bind path="diagnosisItemForm.deletedBy">
			<textarea  cols="60" rows="8" name="<c:out value="${status.expression}"/>"><c:out value="${status.value}" /></textarea>
				<div id="fredcaption">
					<c:out value="${status.errorMessage}" />
				</div>
	    ...
	    <td class="dataField"></td>
			...
			<tr>
				<td class="dataLabel"> Modified Time : </td>				
						...
						<td class="dataField">
			<spring:bind path="diagnosisItemForm.modifiedTime">
					<input type="text" size="35" name="<c:out value="${status.expression}" />" value="<c:out value="${status.value}" />" id="<c:out value="${status.value}"/>" maxlength="19">
					<img src="images/jscalendar.gif" alt="Enter Date" id="<c:out value="${status.expression}"/>_trigger" align="absmiddle" height="20" width="20">
					<script type="text/javascript">
    					Calendar.setup({
	    ...
	    <td class="dataField"></td>
			...
			<td class="dataField">
			<spring:bind path="diagnosisItemForm.modifiedBy">
			<textarea  cols="60" rows="8" name="<c:out value="${status.expression}"/>"><c:out value="${status.value}" /></textarea>
				<div id="fredcaption">
					<c:out value="${status.errorMessage}" />
				</div>
	...
	<tr>
		...
		<td class="dataField">
		<spring:bind path="diagnosisItemForm.diagnosisId">
	...
	<tr>
		...
		<td class="dataField">
		<spring:bind path="diagnosisItemForm.itemId">
		<input type="text" name="<c:out value="${status.expression}"/>" id="<c:out value="${status.expression}"/>" value="<c:out value="${status.value}" />" maxlength="11">
			<div id="fredcaption">
				<c:out value="${status.errorMessage}" />
			</div>
...
<script language="javascript">
	...
	function kembali (){
		...
		document.form1.action = "diagnosisitem";
		document.form1.submit();
	}
	// forreign affairs
		function lookupDiagnosis (){
		window.open ("diagnosis?navigation=lookup&url=diagnosisitem-form","Search","width=700, height=550, menubar=no, status=no, toolbar=no, scrollbars=yes, resizable=yes,screenX=200,screenY=100");
	}
	function setDiagnosis (idx){
		document.form1.method = "POST";
		document.form1.notyet.value="true";
		document.form1.action = "diagnosisitem-form";
		//document.form1.diagnosisItemForm.diagnosisId.value=idx;
		document.getElementById("diagnosisId").value = idx;
		document.form1.submit();
		}
			function lookupItem (){
		window.open ("item?navigation=lookup&url=diagnosisitem-form","Search","width=700, height=550, menubar=no, status=no, toolbar=no, scrollbars=yes, resizable=yes,screenX=200,screenY=100");
	}
	function setItem (idx){
		document.form1.method = "POST";
		document.form1.notyet.value="true";
		document.form1.action = "diagnosisitem-form";
		//document.form1.diagnosisItemForm.itemId.value=idx;
