...
<input type="hidden" name="navigation" value="listmember">
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tbody>
    <tr>
      <td nowrap="nowrap"><h3><img src="images/h3Arrow.gif"  border="0">&nbsp;Member Movement History</h3></td>
        ...
        <tbody>
          <tr>
            ...
            <td class="dataLabel" nowrap="nowrap">Search Category:
			  ...
			  <td class="dataLabel" nowrap="nowrap">Action Type :
              &nbsp;&nbsp;
              
                <select name="actionType">		
                	<option value="">-- SELECT ONE --</option>		
					<option value="ADDITION" <c:if test="${actionType eq 'ADDITION' }">selected</c:if>>ADDITION</option>
					<option value="DELETION" <c:if test="${actionType eq 'DELETION' }">selected</c:if>>DELETION</option>
					<option value="SUSPEND" <c:if test="${actionType eq 'SUSPEND' }">selected</c:if>>SUSPEND</option>
					<option value="REACTIVE" <c:if test="${actionType eq 'REACTIVE' }">selected</c:if>>REACTIVE</option>
					<option value="DELETION" <c:if test="${actionType eq 'DELETION' }">selected</c:if>>DELETION</option>
					<option value="LOSTCARD" <c:if test="${actionType eq 'LOSTCARD' }">selected</c:if>>LOSTCARD</option>
					<option value="REPRINT" <c:if test="${actionType eq 'REPRINT' }">selected</c:if>>REPRINT</option>
					<option value="UPDATE" <c:if test="${actionType eq 'UPDATE' }">selected</c:if>>UPDATE</option>
					<option value="UPGRADE" <c:if test="${actionType eq 'UPGRADE' }">selected</c:if>>UPGRADE</option>
					<option value="MUTATION" <c:if test="${actionType eq 'MUTATION' }">selected</c:if>>MUTATION</option>
					<option value="RENEWAL" <c:if test="${actionType eq 'RENEWAL' }">selected</c:if>>RENEWAL</option>
...
<table class="listView" cellspacing="0" cellpadding="0">
	...
	<tr height="20">
		...
		<td scope="col" class="listViewThS1" nowrap="nowrap" width="5%">Customer Number</td>
	...
	%>
	 <tr  height="20">
		...
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">	
			<c:out value="${memberImport.groupName }"/>
		</td>
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
		...
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top"><c:out value="${memberImport.policyNumber}" /></td>
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top"><c:out value="${memberImport.createdTime}" /></td>
		<%--<td class="oddListRowS1" bgcolor="#e7f0fe"  align="center" valign="top"><c:out value="${memberImport.actionType}" /></td> --%>
		<td class="oddListRowS1" bgcolor="#e7f0fe" valign="top" nowrap="nowrap">
			<c:out value="${memberImport.actionType}" />
			<c:choose>
				<c:when test="${memberImport.actionType eq 'SUSPEND' }">
					<fmt:parseDate value="${memberImport.suspendStart }" var="suspendStartParse" type="both" pattern="EEE MMM dd HH:mm:ss Z yyyy"></fmt:parseDate>
					<fmt:parseDate value="${memberImport.suspendEnd }" var="suspendEndParse" type="both" pattern="EEE MMM dd HH:mm:ss Z yyyy"></fmt:parseDate>
					<fmt:formatDate value="${suspendStartParse }" pattern="dd-MMM-yyyy" var="suspendStartDate"/>
					<fmt:formatDate value="${suspendEndParse }" pattern="dd-MMM-yyyy" var="suspendEndDate"/>
					&nbsp;<b>(<c:out value="${suspendStartDate }"/> s/d <c:out value="${suspendEndDate }"/>)</b>
				</c:when>
				<c:when test="${memberImport.actionType eq 'GRACE' }">
					&nbsp;<b>(<fmt:formatDate pattern="dd-MMM-yyyy" value="${memberImport.graceStartDate }"/> s/d <fmt:formatDate value="${memberImport.graceEndDate }" pattern="dd-MMM-yyyy"/>)</b>
				</c:when>
				<c:otherwise>
				</c:otherwise>
			</c:choose>
...
function exportData(){
	document.form1.action = "memberimport";
	document.form1.navigation.value = "downloadMemberTracking";
	document.form1.submit();
}
