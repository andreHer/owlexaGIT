...
<form name="form1" method="GET" action="providerclaimutilreport" >

 search By :
 <select name="searchby">

 		   			<option name="providerName" <c:if test="${searchby eq \"providerName\"}">selected="true"</c:if> >providerName</option>
			   		   			<option name="providerPeriode" <c:if test="${searchby eq \"providerPeriode\"}">selected="true"</c:if> >providerPeriode</option>
			   		   		   		   		   		   		   		   		   		   		   		   		   		   		   		   </select>
...
<table width="95%" height="25" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<!-- ini default generated table from database -->
		   			<td width="30" align="center">Provider Name</td>
			   		   			<td width="30" align="center">Provider Periode</td>
	...
	<tr>
		...
		<!-- ini default generated table from database -->
		   			<td width="30" align="center"><c:out value="${providerClaimUtilReport.providerName}" /></td>
			   		   			<td width="30" align="center"><c:out value="${providerClaimUtilReport.providerPeriode}" /></td>
			   		   		   		   		   		   		   		   		   		   		   		   		   		   		   		   
		<td width="30" align="center">
			<!-- ini default edit table for each data -->
			<input type="button" name="pilihsaya" onClick="javascript:pilih('<c:out value="${providerClaimUtilReport.id}" />')" value="Pilih">
		</td>
	</tr>
	</c:forEach>
	</table>
...
function pilih (idx){
	window.opener.setProviderClaimUtilReport (idx);
	window.close();
}
function goleft(){
