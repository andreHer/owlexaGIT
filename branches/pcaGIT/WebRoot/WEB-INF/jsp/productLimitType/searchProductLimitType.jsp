...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tbody>
    <tr>
      <td nowrap="nowrap"><h3><img src="images/h3Arrow.gif" alt="Call Search" border="0">&nbsp;Search Product Limit Type</h3></td>
...
<div class="table_container">
	<!-- Table Toolbar Start -->
		...
		<div id="search_float"<%=nampak%>>
			<h4>
				<c:out value="${SEARCH}" /> Data productLimitType
			</h4>
			<form name=form1 onSubmit="cari()">
				...
				<label for="status">
				 ...
				 <select name="searchby" class="inputbox">
 		   			...
 		   			<option name="productLimit" <c:if test="${searchby eq \"productLimit\"}">selected="true"</c:if> >Product Limit</option>
...
<BR/>
				...
				<input type="hidden" name="arah" value="">
								<input type="hidden" name="productLimitTypeId" value="<c:out value="${productLimitType.productLimitTypeId}" />">
							</form>
		</div>
	</div>
	<!-- Table Toolbar Stop -->
...
<LABEL><c:out value="${LIST_OF}" />  Product Limit Type</LABEL>
...
%>
	<c:forEach items="${ProductLimitTypes}" var="productLimitType" varStatus="status" >
	<%	if (i % 2 == 0) {
			rowclass = "col1";
		} else if (i % 2 != 0) {
			rowclass= "col2";
	...
	<tr class="<%=rowclass%>" onmouseover="this.className='highlight'" onmouseout="this.className='<%=rowclass%>'" height="20">
		...
		<!-- ini default generated table from database -->
				...
				<c:out value="${productLimitType.productLimit}" />
			
		</td>
			   			<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" valign="top">
			
				<c:out value="${productLimitType.description}" />
			
		</td>
			   
		<td class="oddListRowS1" bgcolor="#e7f0fe" nowrap="nowrap" align="center">
			<!-- ini default edit table for each data -->
			<a href="javascript:detil('<c:out value="${productLimitType.productLimitTypeId}" />')">
				<img src="images/icon_view.gif" class="action_icon" alt="View" title="View"></a>

			<!-- ini default edit table for each data -->
			<a href="javascript:ubah('<c:out value="${productLimitType.productLimitTypeId}" />')">
				<img src="images/icon_edit.gif" class="action_icon" alt="Edit" title="Edit"></a>

			<!-- ini default delete table for each data -->
			<a href="javascript:hapus('<c:out value="${productLimitType.productLimitTypeId}" />')">
...
function tambahi (){
	...
	document.form1.action = "productlimittype-form";
...
function hapus (idx){
	...
	if (delConfirm){
		...
		document.form1.productLimitTypeId.value = idx;
		document.form1.action = "productlimittype";
